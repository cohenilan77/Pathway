...
export function undergradInterestCluster(profile = {}) {
  ...
  const corpus = raw.join(' ').toLowerCase();
  if (/math|calculus|statistics/.test(corpus) && /coding|computer|hackathon|software|data/.test(corpus)) {
    return ['Computer Science', 'Applied Mathematics', 'Data Science', 'Engineering', 'Business Analytics'];
  }
...
export function undergradBaseline(profile = {}) {
  ...
  const checks = {
    ...
    pathwayType: known(profile.pathwayType),
  ...
  if (grade <= 10) {
    checks.strongestActivity = known(profile.strongestActivity);
    checks.leadership = known(profile.leadership || profile.leadershipEvidence);
    checks.awardsProjects = known(profile.awardsProjects || profile.awards || profile.projects);
    checks.testing = known(profile.tests || profile.testingPlan || profile.testScore);
