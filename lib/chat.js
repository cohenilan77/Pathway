...
// Permanently removes the candidate's chat log and read markers — used when the
// candidate confirms a full session reset ("New session" / refresh).
export async function clearMessages(candidateId) {
  const store = getStore();
  await Promise.allSettled([
    store.del(messagesKey(candidateId)),
    store.del(readAtKey(candidateId, 'candidate')),
    store.del(readAtKey(candidateId, 'staff')),
  ]);
}
