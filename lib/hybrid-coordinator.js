  ...
  async execute({ candidateId, message, action = 'candidate_message', payload = {}, conversationHistory = [], candidateState = {} }) {
    ...
    if (candidateState?.profile?.category !== 'Undergraduate' && isGradSchoolListOverrideRequest(message)) {
      ...
      if (existingPrograms.length >= 10) {
        ...
        return {
          agent: 'matching',
          message: `Here's your school list again — ${existingPrograms.length} schools across reach, target, and likely. Check the University List tab for the full breakdown.`,
          statePatch: { programs: existingPrograms },
          metadata: {
            ...
            fallbackUsed: false, latencyMs: 0, resurfaced: true,
          },
        };
      }
      ...
      const hasEnoughSignalToGenerate = !!candidateState.scores
        || candidateState.profile?.profileCompleteness?.hasStrongProfileBaseline === true
        || candidateState.profile?.candidateFacts?.hasStrongProfileBaseline === true;
