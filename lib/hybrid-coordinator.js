...
const SCHOOL_LIST_UNDERGRAD_RE = /\b(show|give|generate|make|build|see|view)\b[\s\S]{0,60}\b(schools?|universit(?:y|ies)|colleges?|programs?)\b[\s\S]{0,60}\b(list|options|matches?|recommend)\b|\b(show|give|generate|make|build|see|view)\b[\s\S]{0,60}\b(list|options|matches?|recommend)\b[\s\S]{0,60}\b(schools?|universit(?:y|ies)|colleges?|programs?)\b/i;
const SIMPLE_SCHOOL_LIST_RE = /\b(schools?|universit(?:y|ies)|colleges?)\s+list\b|\bmatch\s+me\b|\brecommend(?:ed)?\s+(schools?|universit(?:y|ies)|colleges?)\b|\bwhat\s+schools?\b|\bwhich\s+schools?\b|\bshow\s+me\s+schools?\b/i;
...
// lib/agents/UndergradAgent.js). Defaults OFF: every candidate keeps the
// existing UndergradMasterAgent path, byte-identical, until this is set to
// 'true' in the deployment environment.
export function undergradSmartAgentEnabled(env = process.env) {
  return String(env.UNDERGRAD_SMART_AGENT || '').toLowerCase() === 'true';
}
...
export function isNarrativeChoiceHandoff(message) {
  return NARRATIVE_CHOICE_HANDOFF.test(String(message || ''));
}

export function inferSpecialist(message, action) {
  if (isNarrativeChoiceHandoff(message)) return 'advisor';
...
export async function resolveRoutingDecision({ message, action = 'candidate_message', candidateState = {}, route, enabled = realAgentRouterEnabled() }) {
  ...
  const forceNarrativeAdvisor = isNarrativeChoiceHandoff(message);
  if (!enabled) {
    return { routerSource: 'regex_fallback', routedAgent: forceNarrativeAdvisor ? 'advisor' : forceMatching ? 'matching' : inferSpecialist(message, action), routeIntent: '', routerError: '' };
  }
  try {
    ...
    return {
      ...
      routedAgent: forceNarrativeAdvisor ? 'advisor' : forceMatching ? 'matching' : inferSpecialist(message, action),
      routeIntent: '',
...
export class HybridCoordinator {
  async execute({ candidateId, message, action = 'candidate_message', payload = {}, conversationHistory = [], candidateState = {} }) {
    ...
    if (candidateState?.profile?.category === 'Undergraduate') {
      const looksLikeUndergradSchoolListRequest = SCHOOL_LIST_UNDERGRAD_RE.test(String(message || ''))
        || SIMPLE_SCHOOL_LIST_RE.test(String(message || ''));
      if (looksLikeUndergradSchoolListRequest) {
