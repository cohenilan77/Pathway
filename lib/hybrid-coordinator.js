...
export class HybridCoordinator {
  async execute({ candidateId, message, action = 'candidate_message', payload = {}, conversationHistory = [], candidateState = {} }) {
    ...
    if (candidateState?.profile?.category === 'Undergraduate' && inferSpecialist(message, action) === 'advisor') {
      return runUndergradDeterministicTurn({ candidateId, message, candidateState, conversationHistory });
    }
