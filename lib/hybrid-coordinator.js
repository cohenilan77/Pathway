...
import { canUnlockEssayStage, isNarrativeEligible, isNarrativeStage, shouldStartNarrativeStrategy } from './narrative/strategy.js';
...
export const NARRATIVE_KICKOFF_QUESTION = 'Tell me your story, and why you are here.';
...
// (and essay/interview) entirely for the whole time a candidate is
// mid-narrative-stage. Only these ids — chosen because their
...
//
// 'calendar' is deliberately NOT an escape agent: its pattern
// (deadline|calendar|schedule|event|due date) false-positives on ordinary
// narrative prose about the future/timeline ("hitting every deadline",
// "where I'll be in 5 years"), which was yanking N1-N4 answers over to the
// CalendarAgent mid-story. Deadline/scheduling requests are intentionally
// deferred until the narrative completes and schools are locked.
const NARRATIVE_ESCAPE_AGENTS = ['scholarship', 'essay', 'interview'];
...
export class HybridCoordinator {
    ...
    for (const agentId of plan) {
      ...
      const result = await agent.handle(candidateId, message, {
        ...
        extra: {
          ...
          narrativeText: candidateState.strategy || candidateState.narrativeText,
        },
      });
      lastResult = result;
  ...
  async execute({ candidateId, message, action = 'candidate_message', payload = {}, conversationHistory = [], candidateState = {} }) {
    ...
    if (candidateState?.profile?.category !== 'Undergraduate' && narrativeCoachingV2Enabled()) {
      const currentStrategy = candidateState.strategy || candidateState.narrativeCoaching?.strategy;
      const legacyNarrativeEntry = Array.isArray(candidateState.chosenSchools) && candidateState.chosenSchools.length > 0 && !candidateState.narrativeText;
      const inNarrativeStage = isNarrativeEligible(candidateState)
        && !canUnlockEssayStage(currentStrategy)
        && (isNarrativeStage(candidateState) || shouldStartNarrativeStrategy(candidateState) || isCoachingActive(candidateState) || legacyNarrativeEntry || isNarrativeChoiceHandoff(message, candidateState?.profile?.category));
      ...
      if (inNarrativeStage) {
        ...
        async function runCoachTurn(coachCandidateState) {
          ...
          if (!turn.metadata?.narrativeLocked || !canUnlockEssayStage(turn.statePatch?.strategy)) return turn;
