...
// Short chip-style replies ("ranking improved", "field events", "Math club")
// are answers to the advisor's previous question, not standalone specialist
// requests — but they can accidentally hit a specialist keyword pattern
// (e.g. "ranking improved" matches the search pattern), which would yank an
// Undergraduate conversation turn out of the deterministic advisor path and
// hand it to a database-lookup agent that has no candidate context. Both
// undergrad gates treat these as advisor turns.
export function isChipAnswer(message) {
  const text = String(message || '').trim();
  if (!text || text === '__idle_checkin__') return false;
  const words = text.split(/\s+/).filter(Boolean);
  return text.length <= 40 && words.length <= 4;
}
...
export class HybridCoordinator {
  async execute({ candidateId, message, action = 'candidate_message', payload = {}, conversationHistory = [], candidateState = {} }) {
    ...
    if (candidateState?.profile?.category === 'Undergraduate'
      ...
      && (inferSpecialist(message, action) === 'advisor' || (isChipAnswer(message) && (!action || action === 'candidate_message')))) {
