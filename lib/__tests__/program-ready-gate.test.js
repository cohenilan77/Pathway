...
test('ready message is allowed only when a validated program list exists', () => {
  ...
  assert.equal(gateProgramReadyReply({ text, isUndergrad: true, parsedPrograms: programs(1), currentPrograms: [] }), PROGRAM_RECOVERY_REPLY);
});
