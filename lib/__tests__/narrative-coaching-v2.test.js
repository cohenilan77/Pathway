...
import { canUnlockEssayStage, isSubstantiveConfirmation, shouldStartNarrativeStrategy, validateStrategy } from '../narrative/strategy.js';
...
const COMPLETE_STRATEGY = {
  version: 1, status: 'ready_for_confirmation', confirmationStatus: 'unconfirmed',
  goals: { shortTerm: 'Product manager in climate software', longTerm: 'Build climate adaptation products' },
  educationLogic: { whyDegree: 'Close product and commercialization gaps', whyNow: 'Current role has reached its scope ceiling' },
  transition: { type: 'adjacent', archetype: 'Function shift', distanceScore: 1 },
  admissionsProposition: 'An energy operator with measured delivery evidence moving into climate product leadership.',
  proofInventory: [{ claim: 'Led an internal product pilot', source: 'CV' }],
  memoryTag: 'energy operator becoming climate product builder',
};

test('strategy workflow starts from canonical Narrative stage without a literal handoff sentence', () => {
  assert.equal(shouldStartNarrativeStrategy({
    stageName: 'Narrative', profile: { category: 'Graduate', workExperience: [{ role: 'Engineer' }] }, chosenSchools: [],
  }), true);
});

test('undergraduate candidates cannot enter strategy workflow', () => {
  assert.equal(shouldStartNarrativeStrategy({ stageName: 'Narrative', profile: { category: 'Undergraduate', education: [{}] } }), false);
});

test('polite confirmation is rejected and substantive confirmation is accepted', () => {
  assert.equal(isSubstantiveConfirmation('Looks good'), false);
  assert.equal(isSubstantiveConfirmation('This feels true because it captures my energy experience and product goal; I confirm it.'), true);
});

test('essay unlock requires a complete confirmed strategy', () => {
  assert.equal(canUnlockEssayStage(COMPLETE_STRATEGY), false);
  assert.equal(canUnlockEssayStage({ ...COMPLETE_STRATEGY, status: 'confirmed', confirmationStatus: 'confirmed' }), true);
});

test('malformed ready strategy is rejected deterministically', () => {
  const result = validateStrategy({ status: 'ready_for_confirmation', confirmationStatus: 'unconfirmed', goals: {} });
  assert.equal(result.valid, false);
  assert.ok(result.errors.includes('goals.shortTerm'));
});

test('strategy tools persist, revise, and require substantive confirmation', async () => {
  const ctx = { now: 123, evidenceSnapshot: {}, workingStrategy: null, workingNarrativeText: '', strategyDirty: false, narrativeTextDirty: false };
  const saved = await executeNarrativeCoachTool(toolUse('save_strategy_draft', { strategy: COMPLETE_STRATEGY }), ctx);
  assert.equal(saved.status, 'saved');
  const polite = await executeNarrativeCoachTool(toolUse('confirm_strategy', { candidateResponse: 'Yes' }), ctx);
  assert.equal(polite.error, 'substantive_confirmation_required');
  const confirmed = await executeNarrativeCoachTool(toolUse('confirm_strategy', { candidateResponse: 'This feels true because it reflects my real transition and evidence; I confirm it.' }), ctx);
  assert.equal(confirmed.status, 'confirmed');
  assert.equal(ctx.workingStrategy.confirmationStatus, 'confirmed');
  assert.ok(ctx.workingNarrativeText);
});
...
test('a narrative answer that mentions deadlines/scheduling stays with the coach and is NOT escaped to CalendarAgent', async () => {
  process.env.NARRATIVE_COACHING_V2 = 'true';
  let calledWith = null;
  const original = NarrativeCoachAgent.prototype.handle;
  NarrativeCoachAgent.prototype.handle = async function stub(candidateId, message, opts) {
  ...
  try {
    const coordinator = new HybridCoordinator();
    const result = await coordinator.execute({
      candidateId: 'cand_deadline',
      // A genuine N-answer whose prose happens to contain calendar-pattern words
      // (deadline / schedule). Before the fix this matched the 'calendar'
      // SPECIALIST_PATTERN and escaped the coach into CalendarAgent mid-story.
      message: 'The moment that convinced me was leading a launch on a brutal deadline, owning the schedule end to end.',
      candidateState: {
        profile: { category: 'Graduate' },
        chosenSchools: ['Wharton'],
        narrativeCoaching: { rawGoal: 'Product leadership', sessionContext: { pivotRisk: null, schoolContext: null, outcomeContext: null } },
      },
    });
    assert.ok(calledWith, 'the deadline-mentioning narrative answer should still reach NarrativeCoachAgent');
    assert.equal(result.agent, 'NarrativeCoachAgent');
    assert.notEqual(result.agent, 'CalendarAgent');
  } finally {
    NarrativeCoachAgent.prototype.handle = original;
    delete process.env.NARRATIVE_COACHING_V2;
  }
