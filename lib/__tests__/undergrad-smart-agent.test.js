...
test('UNDERGRAD_TOOLS registers the smart undergrad toolset', () => {
  const names = UNDERGRAD_TOOLS.map(t => t.name).sort();
  assert.deepEqual(names, [
    'cache_scholarship_results', 'end_turn_response', 'find_community_group', 'get_candidate_state', 'get_document',
    'lookup_school_data', 'lookup_scholarships', 'save_calendar_event', 'save_community_join_intent',
    'save_essay_material', 'save_profile_fact', 'save_progress_snapshot',
    'save_scholarship_interest', 'save_session_summary', 'save_task', 'save_university_list',
  ].sort());
});
...
test('explicit env false routes Undergraduate turns to UndergradMasterAgent fallback, not UndergradAgent', async () => {
  process.env.UNDERGRAD_SMART_AGENT = 'false';
...
test('default routes Undergraduate turns to UndergradAgent', async () => {
  delete process.env.UNDERGRAD_SMART_AGENT;
  let calledMaster = false;
  let calledSmart = false;
  const originalMasterHandle = UndergradMasterAgent.prototype.handle;
...
test('MainAgent authenticated path catches undergrad school-list command before old question flow', async () => {
  const agent = new MainAgent();
  const result = await agent.handle('cand_main_school_list', 'list shcools', {
    conversationHistory: [{ role: 'user', text: 'Biology or life sciences' }],
    extra: {
      profile: { category: 'Undergraduate', grade: '10', gpa: 3.6 },
      programs: [],
    },
  });
  assert.equal(result.intent, 'undergrad_school_list');
  assert.equal(result.result.statePatch.profile.intendedMajor, 'Life Sciences');
  assert.equal(result.result.statePatch.programs.length, 12);
  assert.match(result.result.text, /Reach:/);
});
...
// (School-list-shaped phrasing is deliberately excluded from this loop —
// the 2026-07-09 school-list hotfix now intercepts those before UndergradAgent
// even runs; see the dedicated hotfix test below.)
...
test('explicit env false: a profile-reveal request still escapes to the generic advisor path fallback', async () => {
  process.env.UNDERGRAD_SMART_AGENT = 'false';
  let calledMaster = false;
  let calledSmart = false;
  const originalMasterHandle = UndergradMasterAgent.prototype.handle;
