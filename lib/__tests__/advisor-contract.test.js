...
test('scholarships survives validateStatePatch instead of being silently dropped as an unrecognized field', () => {
  const scholarships = [{ name: 'Founders Award', url: 'https://example.com/founders', status: 'interested' }];
  assert.deepEqual(validateStatePatch({ scholarships }).scholarships, scholarships);
});
...
test('natural interview phrasing routes to the interview specialist, not just "mock interview"/"interview prep"', () => {
  // Previously required the exact phrase "mock interview", "interview prep",
  // or "evaluate my answer" — everyday phrasing like "practice interview
  // questions" or "help me with my upcoming interview" fell through to the
  // generic advisor instead of InterviewAgent.
  assert.equal(inferSpecialist('Can you give me some interview questions to practice?'), 'interview');
  assert.equal(inferSpecialist('I have an interview next week, help me prepare'), 'interview');
  assert.equal(inferSpecialist('Let\'s do a practice interview'), 'interview');
  assert.equal(inferSpecialist('interview'), 'interview');
});
