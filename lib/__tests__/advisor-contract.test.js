...
import { buildExecutionPlan, buildExecutionPlanForAgent, inferSpecialist, isChipAnswer, isOnboardingContinuation, looksLikeProfileText, realAgentRouterEnabled, resolveRoutingDecision, wantsProfileAnalysis } from '../hybrid-coordinator.js';
...
test('chip-style answers are recognized so undergrad turns stay with the advisor', () => {
  // Short replies to the advisor's question, even when they trip a
  // specialist keyword pattern ("ranking" matches the search pattern).
  assert.equal(isChipAnswer('ranking improved'), true);
  assert.equal(isChipAnswer('field events'), true);
  assert.equal(isChipAnswer('Math club'), true);
  assert.equal(isChipAnswer('10th grade'), true);
  // Sanity: the misrouting this guards against actually exists.
  assert.equal(inferSpecialist('ranking improved'), 'search');
  // Genuine requests and non-answers are not chip answers.
  assert.equal(isChipAnswer('Please recommend a full university list for my profile and interests'), false);
  assert.equal(isChipAnswer('__idle_checkin__'), false);
  assert.equal(isChipAnswer(''), false);
});
