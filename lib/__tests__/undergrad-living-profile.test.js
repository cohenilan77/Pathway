...
const grade10 = {
  ...
  strongestActivity: 'hackathon', leadership: 'Club Leader', awardsProjects: 'None', tests: 'None yet', universityStyle: 'Top ranked',
};
...
test('Grade 10 exploring profile can score and generate programs without intended major or test score', () => {
  ...
  assert.deepEqual(result.interestCluster, ['Computer Science', 'Applied Mathematics', 'Data Science', 'Engineering', 'Business Analytics']);
...
test('interest clusters are inferred from subjects and activities', () => {
  assert.deepEqual(undergradInterestCluster({ subjects: ['Math'], activities: ['Coding club'] }), ['Computer Science', 'Applied Mathematics', 'Data Science', 'Engineering', 'Business Analytics']);
});
