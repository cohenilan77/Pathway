...
import { runUndergradDeterministicTurn } from '../undergrad/deterministic-turn.js';
...
export class MainAgent extends BaseAgent {
  ...
  async handle(candidateId, message, { conversationHistory = [], extra = {}, forcedAgent = null } = {}) {
    ...
    if (!forcedAgent && extra?.profile?.category === 'Undergraduate') {
      const { inferSpecialist } = await import('../hybrid-coordinator.js');
      if (inferSpecialist(message) === 'advisor') {
        const turn = await runUndergradDeterministicTurn({
          candidateId,
          message,
          candidateState: {
        ...
        await logAgentInteraction(candidateId, turn.agent, 'Undergrad deterministic stage tracker', {
