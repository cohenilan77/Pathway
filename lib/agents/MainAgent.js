...
export class MainAgent extends BaseAgent {
  ...
  async handle(candidateId, message, { conversationHistory = [], extra = {}, forcedAgent = null } = {}) {
    ...
    if (!forcedAgent && extra?.profile?.category === 'Undergraduate') {
      const { inferSpecialist, isChipAnswer, wantsProfileAnalysis } = await import('../hybrid-coordinator.js');
      ...
      // have no candidate context to answer them with. Profile-reveal
      // requests ("ready to see my full profile") skip the deterministic
      // path instead: only the full AdvisorAgent below can emit the
      // <PROFILE>/<SCORES> analysis blocks the app renders.
      if (!wantsProfileAnalysis(message) && (inferSpecialist(message) === 'advisor' || isChipAnswer(message))) {
