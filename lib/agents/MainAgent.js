...
import { AdvisorAgent, ADVISOR_RETRY_BUDGET_MS } from './sub/AdvisorAgent.js';
    ...
    switch (agentName) {
      case 'advisor': {
        ...
        const rawText = await agent.chat(advisorMessages, {
          ...
          deadlineAt: startedAt + ADVISOR_RETRY_BUDGET_MS,
      ...
      case 'matching':
        result = await agent.match(candidateId, extra.preferences, extra.profile);
        break;
      case 'profile':
      ...
      case 'community':
        ...
        break;
      case 'scholarship': {
        // Search by the candidate's own list: their confirmed target schools
        // (chosenSchools, surfaced by the coordinator as targetSchools) first,
        // then their recommended programs. Without this the agent had no
        // school list and could only do a generic profile search, so the
        // Scholarships tab never filled from the candidate's actual programs.
        const scholarshipSchools = extra.targetSchools || extra.chosenSchools
          || (extra.programs || []).map(p => p?.name).filter(Boolean);
        result = extra.searchScholarships
          ? await agent.search(candidateId, extra.profile || {}, scholarshipSchools)
          : await agent.handle(candidateId, message, extra.profile || {}, scholarshipSchools);
        break;
      }
      case 'document':
        if (extra.upload) {
