...
import { SCHOLARSHIP_TOOLS, lookupScholarships, cacheScholarshipLookup, normalizeScholarshipSave } from './scholarship-tools.js';
...
// volunteeredDemographics: ONLY ever written when the student raises it
// themselves ("I'm looking for Hispanic scholarships") — never asked for
// directly. There is no automatic demographic filter to gate (scholarship
// data isn't a static list — see lib/scholarships/scholarship-cache.js);
// this field exists so the agent can ground a demographic-specific
// lookup_scholarships query in something the student actually said, never
// in an inference. See the privacy rule in DEFAULT_UNDERGRAD_PROMPT below.
...
async function lookupScholarshipsTool(ctx, { schoolName, query } = {}) {
  return lookupScholarships({ schoolName, query });
}

async function cacheScholarshipResultsTool(ctx, input) {
  return cacheScholarshipLookup(input);
}

// Deadline hook: a saved scholarship carrying a real ISO deadline also
// becomes a calendar event, so the existing nagger/reminder machinery picks
// it up before it closes — same pattern save_calendar_event already uses.
function saveScholarshipInterestTool(ctx, input) {
  const { error, record } = normalizeScholarshipSave(input, ctx.now);
  if (error) return { error };
  const dedupeKey = record.name.toLowerCase();
  ctx.workingScholarships = [
    ...ctx.workingScholarships.filter(s => s.name.toLowerCase() !== dedupeKey),
    record,
  ];
  ctx.scholarshipsDirty = true;
  if (record.deadline && isFutureIsoDate(record.deadline, ctx.now)) {
  ...
  if (!ctx.primaryArea) ctx.primaryArea = 'scholarships';
...
export async function executeUndergradTool(candidateId, toolUse, ctx) {
  ...
  try {
    switch (toolUse?.name) {
      ...
      case 'lookup_scholarships': return await lookupScholarshipsTool(ctx, input);
      case 'cache_scholarship_results': return await cacheScholarshipResultsTool(ctx, input);
      case 'save_scholarship_interest': return saveScholarshipInterestTool(ctx, input);
