...
import { lookupScholarships, cacheScholarshipLookup, normalizeScholarshipSave, SCHOLARSHIP_TOOLS, SAVE_SCHOLARSHIP_RESULTS_TOOL } from '../scholarship-tools.js';

test('SCHOLARSHIP_TOOLS defines lookup, cache, and save (shared trio, bulk tool kept out)', () => {
  assert.deepEqual(
...
test('SAVE_SCHOLARSHIP_RESULTS_TOOL is a separate grad-only bulk tool, not in the shared SCHOLARSHIP_TOOLS array', () => {
  assert.ok(!SCHOLARSHIP_TOOLS.some(t => t.name === 'save_scholarship_results'), 'must stay out of the shared array so the Undergraduate toolset is unchanged');
