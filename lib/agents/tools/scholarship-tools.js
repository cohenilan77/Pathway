...
// Bulk "save a whole search's grounded results to the candidate's tab" tool.
// Kept OUT of the shared SCHOLARSHIP_TOOLS array so the Undergraduate smart
// agent's toolset (which spreads SCHOLARSHIP_TOOLS) stays exactly as it was;
// only the Graduate ScholarshipAgent binds this one, alongside its own
// handleToolUse for it.
export const SAVE_SCHOLARSHIP_RESULTS_TOOL = {
  name: 'save_scholarship_results',
  description: 'Persist the grounded scholarships you found this turn (from lookup_scholarships or a fresh web_search) to the candidate\'s Scholarships tab as suggestions. Call this ONCE with every verified match after you have searched each chosen school and the field/background query. Each entry needs a real name and source url — entries missing either are dropped, not saved. These are saved with status "suggested"; use save_scholarship_interest instead for a scholarship the candidate explicitly says they want.',
  input_schema: {
    type: 'object',
    properties: {
      scholarships: {
...
export function normalizeScholarshipSave({ name, url, amountUSD, amountType, eligibility, deadline, status } = {}, now = Date.now()) {
  ...
  return {
    record: {
      ...
      eligibility: eligibility ? String(eligibility).slice(0, 500) : null,
...
// Bulk counterpart used by save_scholarship_results: normalizes a batch of
// grounded search results into persistable records with status "suggested",
// dropping any missing name+url and de-duping by name+url so the same
// scholarship from two schools' searches is stored once. Never throws.
export function normalizeScholarshipResults(scholarships = [], now = Date.now()) {
  const seen = new Set();
  const records = [];
  for (const entry of Array.isArray(scholarships) ? scholarships : []) {
    const { record } = normalizeScholarshipSave({ ...entry, status: entry?.status || 'suggested' }, now);
    if (!record) continue;
    const dedupeKey = `${record.name.toLowerCase()}|${record.url.toLowerCase()}`;
