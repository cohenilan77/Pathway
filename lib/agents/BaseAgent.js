...
export class BaseAgent {
  ...
  async execute(messages, { tools = [], context = {}, toolChoice = null } = {}) {
    let runtime = null;
    ...
    if (allowedTools.length) {
      params.tools = allowedTools;
      // Only honor a forced tool_choice when that tool actually survived the
      // admin-config filter above; otherwise leave Claude free to respond in
      // text (matches every other agent's unforced default).
      if (toolChoice && allowedTools.some(tool => tool.name === toolChoice.name)) params.tool_choice = toolChoice;
    }
