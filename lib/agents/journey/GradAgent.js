...
export class GradAgent extends BaseAgent {
  ...
  async handleToolUse(toolUse) {
    ...
    if (toolUse.name === 'build_portfolio') {
      ...
      console.log(`[GradAgent] build_portfolio: collected ${deduped.length} unique tasks for ${candidateId} from ${schools?.length || 0} schools`);
      await patchJourney(candidateId, {
        collected: { portfolio: normalized, pendingTasks: deduped },
        flags: { programsShown: true, chosenSchools: schools || [], stage: 'portfolio' },
      });
      return { programs: normalized, tasks: deduped };
    }
  ...
  async chat(candidateId, message, { conversationHistory = [], profile = {}, scores = {}, kpiSummary = '' } = {}) {
    ...
    if (finalJourney.collected.pendingTasks?.length && !text.includes('<TASKS>')) {
      console.log(`[GradAgent] Emitting ${finalJourney.collected.pendingTasks.length} tasks for ${candidateId}`);
      blocks.push(`<TASKS>${JSON.stringify(finalJourney.collected.pendingTasks)}</TASKS>`);
    } else if (!finalJourney.collected.pendingTasks?.length) {
      console.log(`[GradAgent] No pending tasks found for ${candidateId} (toolCalls: ${this.runtime.toolCalls.join(', ')})`);
    }
