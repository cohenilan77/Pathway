...
function extractSubmittedProfileFacts(text) {
  const source = String(text || '');
  const gpa = source.match(/\bgpa\s*[:=-]?\s*(\d(?:\.\d{1,2})?)/i)?.[1];
  const gmat = source.match(/\bgmat\s*[:=-]?\s*(\d{3})\b/i)?.[1];
  const gre = source.match(/\bgre\s*[:=-]?\s*(\d{3})\b/i)?.[1];
  const experience = source.match(/\b(\d+(?:\.\d+)?)\s*(?:years?|yrs?)\s+(?:of\s+)?(?:work|professional|industry)?\s*experience\b/i)?.[1];
  return {
    ...(gpa ? { gpa: Number(gpa) } : {}),
    ...(gmat ? { gmat: Number(gmat), testScore: Number(gmat) } : {}),
    ...(gre ? { gre: Number(gre), testScore: Number(gre) } : {}),
    ...(experience ? { experience: `${experience} years` } : {}),
  };
}

function missingProfileFollowup(journey) {
  const collected = journey?.collected || {};
  const missing = [];
  if (!hasProfileEvidence(collected, /recommender|recommendation|supervisor|professor|manager/)) missing.push('your likely recommenders and how directly they evaluated your work');
  if (!hasProfileEvidence(collected, /goal|target role|career|research direction|post-mba|post-degree/)) missing.push('your post-degree career or research goal');
  if (!hasProfileEvidence(collected, /destination|location|geograph|countr|region|usa|united states|europe|uk|canada/)) missing.push('your target country or region');
  if (!hasProfileEvidence(collected, /mba|master|msc|ma\b|phd|doctoral|program|field of study/)) missing.push('the exact degree or field you want');
  const request = missing.length
    ? `I received and saved your file. To complete the profile analysis, tell me ${missing.slice(0, 3).join(', ')}.`
    : 'I received and saved your file. I am analyzing the profile now; what important achievement or context is missing from the document?';
  return `${request} -> Add missing details | Nothing else to add`;
}
...
export class GradAgent extends BaseAgent {
  ...
  async chat(candidateId, message, { conversationHistory = [], profile = {}, scores = {}, programs = [], kpiSummary = '' } = {}) {
    let journey = await getJourney(candidateId);
    const submittedProfile = /^Here is my CV\/resume:/i.test(String(message || '').trim());
    if (submittedProfile) {
      journey = await patchJourney(candidateId, {
        collected: {
          ...extractSubmittedProfileFacts(message),
          cvText: String(message || ''),
          hasCV: true,
          profileSourceReceivedAt: new Date().toISOString(),
        },
        flags: { stage: 'profile' },
      });
    }
    ...
    if (finalJourney.collected.hasCV && /(?:upload|attach|paste|send|share).{0,30}(?:cv|resume|transcript|background|file)/i.test(text)) {
      text = missingProfileFollowup(finalJourney);
    }
    const blocks = [];
