...
export function applyHardGates(program = {}) {
  const candidateGPA = Number(program.candidateGPA);
  const candidateTest = Number(program.candidateTest);
  const medianGPA = Number(program.medianGPA);
  const medianTest = Number(program.medianTest);
  const hasGpa = Number.isFinite(candidateGPA) && Number.isFinite(medianGPA);
  const hasTest = Number.isFinite(candidateTest) && Number.isFinite(medianTest);
  const gpaGap = hasGpa ? candidateGPA - medianGPA : 0;
  const testGap = hasTest ? candidateTest - medianTest : 0;
  const locked = isLocked(
    hasGpa ? candidateGPA : null,
    hasTest ? candidateTest : null,
    hasGpa ? medianGPA : null,
    hasTest ? medianTest : null,
  );
  const originalFit = Number(program.fit);
  const fit = Number.isFinite(originalFit) ? realismCap(originalFit, gpaGap, testGap) : null;
  const gateFlags = [];
  if (gpaGap < -LOCKED_GPA_GAP) gateFlags.push('gpa_below_gate');
  if (testGap < -LOCKED_TEST_GAP) gateFlags.push('test_below_gate');
  return {
    ...program,
    fit,
    locked,
    tier: locked ? 'locked' : program.tier,
    riskFlags: [...new Set([...(program.riskFlags || []), ...gateFlags])],
  };
}
