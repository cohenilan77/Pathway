...
test('a "Goodbye" option is available on wind-down (not tacked onto every turn) and wraps the session up', () => {
  assert.match(DEFAULT_UNDERGRAD_PROMPT, /GOODBYE ALWAYS AVAILABLE/i);
  assert.match(DEFAULT_UNDERGRAD_PROMPT, /Only OFFER a "Goodbye 👋" option when a session is naturally winding down/i);
  assert.match(DEFAULT_UNDERGRAD_PROMPT, /Do NOT tack a "Goodbye" chip onto every single turn/i);
