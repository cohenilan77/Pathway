...
function narrativeContextBlock(narrative) {
  if (!narrative) return '';
  if (typeof narrative === 'string') return `CANDIDATE NARRATIVE (source of truth for all essays):\n"${narrative}"\n\nEvery draft, review, and revision must be consistent with this narrative. If the candidate's request conflicts with it, note the conflict and ask before drifting.\n\n`;
  if (narrative.confirmationStatus !== 'confirmed' || narrative.status !== 'confirmed') {
    return 'STRATEGY GATE: The candidate strategy is not confirmed. Do not draft or rewrite an essay; explain that substantive strategy confirmation is required.\n\n';
  }
  const safe = {
    goals: narrative.goals,
    transition: narrative.transition,
    admissionsProposition: narrative.admissionsProposition,
    recommendedStrategy: narrative.recommendedStrategy,
    emphasisMap: narrative.emphasisMap,
    riskRegister: narrative.riskRegister,
    proofInventory: narrative.proofInventory,
    memoryTag: narrative.memoryTag,
    schoolStrategyCards: narrative.schoolStrategyCards,
  };
  return `CONFIRMED STRATEGY — IMMUTABLE INSTRUCTION SET:\n${JSON.stringify(safe, null, 2)}\n\nDo not change the candidate's goal, invent a narrative, or reclassify the transition. Use the emphasis map, risks, proof, and relevant school card. Adapt school emphasis without changing core identity. Flag contradictions instead of silently rewriting them.\n\n`;
}
