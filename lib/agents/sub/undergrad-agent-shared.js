...
export class BaseUndergradAgent extends BaseAgent {
  ...
  async respond(candidateId, message, {
    ...
    // The forced respond_with_options tool call is what keeps every undergrad
    // reply in the chip-question shape the app renders. If it wasn't used
    // (e.g. an admin config disabled the tool, or the model produced free
    // text despite the forced tool_choice), fall back to a graceful, honest
    // message rather than surfacing whatever raw text the model wrote —
    // undergrad conversational agents never have profile-lookup tooling, so
    // unvetted raw text risks a hallucinated "I don't have access to a
    // stored profile" apology instead of a normal next question.
    const fallbackUsed = !structured;
    return {
      text: structured?.message ?? "Let's keep going — tell me a bit more, or pick what fits best.",
      options: structured?.options ?? ['Tell me more', 'Not sure', 'Skip for now'],
      fallbackUsed,
      topic,
