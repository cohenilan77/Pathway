...
export class AdvisorAgent {
  ...
  async chat(messages, { systemPrompt, onAttempt, formatConstraint = null, hasPrograms = false, deadlineAt = Date.now() + ADVISOR_RETRY_BUDGET_MS } = {}) {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      // Web search only helps on the first attempt's fresh generation — by
      // attempt 1+ we're already in a corrective retry (missing blocks, bad
      // format mix, clustered portfolio) where speed matters more than
      // re-searching, and it was adding real latency on the path most likely
      // to already be short on time.
      const useWebSearch = attempt === 0;
      ...
      const hasProgramsBlock = /<PROGRAMS>[\s\S]*?<\/PROGRAMS>/.test(raw) && (!mustGeneratePrograms || programsCount >= 10);
    ...
    if (mustGeneratePrograms && hasAnyProgramsBlock && finalProgramsCount < 10) {
      // Exhausted all 4 attempts still short of the 10-school minimum for a
      ...
      console.warn(`[AdvisorAgent] Shipping a short portfolio (${finalProgramsCount}/10 minimum) after exhausting all retries.`);
