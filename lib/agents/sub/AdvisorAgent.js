...
export function requestsProgramList(text) {
  return /(?:recommend|generate|regenerate|show|build|create|refresh)[\s\S]{0,80}(?:programs?|portfolio|schools?|school\s+list|matches)/i.test(String(text || ''))
    || /(?:cannot|can't|do not|don't)\s+see[\s\S]{0,80}(?:programs?|portfolio|schools?|list|matches)/i.test(String(text || ''));
}
...
export class AdvisorAgent {
  ...
  async chat(messages, { systemPrompt, onAttempt, formatConstraint = null } = {}) {
    ...
    const mustGeneratePrograms = requestsProgramList(lastUserContent);
