...
export class AdvisorAgent {
  ...
  async chat(messages, { systemPrompt, onAttempt } = {}) {
    let raw;
    let retryReason = '';
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      const badPortfolioMix = hasRequiredBlocks && hasProgramsBlock && !isAdvanceTurn
        && !/<CHOSEN_SCHOOLS>/i.test(raw) && this.needsPortfolioMixRetry(raw);
