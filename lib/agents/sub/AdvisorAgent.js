...
import { MIN_GENERATED_PROGRAMS } from '../../program-validation.js';
...
// Wall-clock budget for chat()'s attempt loop below. Mirrors vercel.json's
// maxDuration for api/advisor.js and api/chat.js (280s on the current Pro
// plan) minus a margin for request setup, DB/metric writes, and response
// serialization that happen outside this loop. Callers that already track
// their own request start time (MainAgent.handle, api/chat.js's handler)
// should derive an explicit deadlineAt from it instead of relying on this
// default, since chat() itself doesn't know how much of the budget the
// caller already spent before invoking it.
export const ADVISOR_RETRY_BUDGET_MS = 260_000;
...
const MAX_TOKENS_COMPACT_NOTE = '\n\nCOMPACT RETRY: The previous response hit the output-token limit. Return the required structured blocks only, with no prose before them. For a portfolio request, return one complete <PROGRAMS> JSON block. Keep every program object valid but compact, and keep each PhD programInfo to approximately 45-70 words so the closing </PROGRAMS> tag fits within the output limit.';
...
export function claimsPortfolioReady(raw) {
  ...
  return /(?:portfolio|programs?|school(?:\s+list)?|university\s+list|shortlist)[\s\S]{0,180}(?:analysis\s+tab|already\s+shared|ready|matched|full\s+list)/i.test(text)
    ...
    || /(?:matched\s+\d+|\d+[- ]school\s+portfolio|portfolio\s+spans|realistic\s+reaches|reach(?:es)?[,/ ]+targets?[,/ ]+(?:and\s+)?likel(?:y|ies))/i.test(text);
}

export function requestsProgramList(text, { hasPrograms = false } = {}) {
  const value = String(text || '').trim();
  return /(?:recommend|generate|regenerate|show|build|create|refresh|list|see)[\s\S]{0,80}(?:programs?|portfolio|schools?|school\s+list|university\s+list|matches)/i.test(value)
    ...
    || (!hasPrograms && (/(?:^|\b)continue\s+(?:with\s+)?(?:my\s+)?analysis\b/i.test(value) || /^(?:next|continue|proceed|move on)[.!]?$/i.test(value)));
}
...
export class AdvisorAgent {
  ...
  async chat(messages, { systemPrompt, onAttempt, formatConstraint = null, hasPrograms = false, deadlineAt = Date.now() + ADVISOR_RETRY_BUDGET_MS } = {}) {
    let raw;
    let bestProgramsRaw = '';
    let retryReason = '';
    ...
    const undergradReadyForPrograms = !hasPrograms
      && /"selectedCandidateType"\s*:\s*"Undergraduate"/i.test(String(systemPrompt || ''))
      && /"readyForPrograms"\s*:\s*true/i.test(String(systemPrompt || ''));
    const mustGeneratePrograms = requestsProgramList(lastUserContent, { hasPrograms }) || undergradReadyForPrograms;

    for (let attempt = 0; attempt < 4; attempt++) {
      const elapsedMs = Date.now() - loopStartedAt;
      if (attempt > 0 && Date.now() >= deadlineAt) {
        console.error(`[AdvisorAgent] Retry budget exhausted before attempt ${attempt} (elapsed=${elapsedMs}ms); shipping best response so far instead of retrying.`);
        break;
      }
      console.error(`[AdvisorAgent] attempt ${attempt} start, elapsed=${elapsedMs}ms`);
      const retryNote = attempt === 0
        ? ''
        : retryReason === 'max_tokens'
          ? MAX_TOKENS_COMPACT_NOTE
      ...
      // Forced portfolio generation uses the saved profile and skips search
      // so the full token/time budget is available for the structured block.
      // Other turns may search on attempt 0, but never on corrective retries.
      const useWebSearch = attempt === 0 && !mustGeneratePrograms;
      ...
      const programsCount = this.parseProgramsFromRaw(raw).length;
      if (programsCount > 0) bestProgramsRaw = raw;
      const hasProgramsBlock = /<PROGRAMS>[\s\S]*?<\/PROGRAMS>/.test(raw) && (!mustGeneratePrograms || programsCount >= MIN_GENERATED_PROGRAMS);
      ...
      if (response.stop_reason === 'max_tokens' && !usedCompactRetry) {
        retryReason = 'max_tokens';
    ...
    if (mustGeneratePrograms && this.parseProgramsFromRaw(raw).length === 0 && bestProgramsRaw) raw = bestProgramsRaw;
    raw = this.normalizeProgramsInRaw(raw);

    const claimsPortfolioLive = claimsPortfolioReady(raw) || mustGeneratePrograms;
    const claimsAnalysisReady = /Your analysis is ready/i.test(raw);
    const finalProgramsCount = this.parseProgramsFromRaw(raw).length;
    const hasAnyProgramsBlock = /<PROGRAMS>[\s\S]*?<\/PROGRAMS>/.test(raw) && finalProgramsCount > 0;
    const hasAnalysisBlocks = /<PROFILE>[\s\S]*?<\/PROFILE>/.test(raw)
      && /<SCORES>[\s\S]*?<\/SCORES>/.test(raw)
      && /<STRENGTHS>[\s\S]*?<\/STRENGTHS>/.test(raw)
      && /<WEAKNESSES>[\s\S]*?<\/WEAKNESSES>/.test(raw)
    ...
    if (mustGeneratePrograms && hasAnyProgramsBlock && finalProgramsCount < MIN_GENERATED_PROGRAMS) {
      // Exhausted all 4 attempts still short of the generated-school minimum for a
      // Branch B (recommended, not candidate-named) portfolio. Shipping the
      // real, if short, portfolio beats discarding it for a generic
      // non-answer — but this must never happen silently.
      console.warn(`[AdvisorAgent] Shipping a short portfolio (${finalProgramsCount}/${MIN_GENERATED_PROGRAMS} minimum) after exhausting all retries.`);
    } else if (claimsPortfolioLive && !hasAnyProgramsBlock) {
      raw = 'I saved your profile. Your next step is to choose one academic or activity priority.';
    }
