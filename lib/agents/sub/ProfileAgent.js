...
Never invent GPA, test scores, work years, achievements, or missing dates. Uploaded file text, pasted profile text, and additional text are one factual baseline. Unknown facts must be null, marked incomplete, and listed in missingFields; unknown never means weak.
...
export class ProfileAgent extends BaseAgent {
  ...
  async parse(candidateId, rawText) {
    ...
    const messages = [
      {
        role: 'user',
        content: `Parse the complete profile source bundle below now. First detect the language of each populated source section and translate all sections to English internally. Then merge the English meaning of UPLOADED FILE TEXT, PASTED CV / FIRST TEXT BOX, and ADDITIONAL TEXT / SECOND TEXT BOX into one factual baseline. Extract every fact before identifying gaps; never ask the candidate to upload or provide the document again. Save all free-text profile fields in English and include sourceLanguages, normalizationLanguage:"English", and sourceCoverage. Then call update_profile. After the tool succeeds, return strict JSON with keys "profile" (the English-normalized extracted fields), "missingFields" (only information genuinely absent from every supplied section), and "summary" (English). Do not ask for information already present in any language or section.\n\nCandidate ID: ${candidateId}\n\nRaw content:\n${rawText}`,
      },
    ];
