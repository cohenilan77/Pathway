...
export async function toEnglish(text, userId = 'anonymous', conversationId = 'legacy_session') {
...
export async function fieldsToEnglish(fields, userId = 'anonymous', conversationId = 'legacy_session') {
  const translated = await Promise.all(fields.map((f) => toEnglish(f, userId, conversationId)));
  return translated;
}
