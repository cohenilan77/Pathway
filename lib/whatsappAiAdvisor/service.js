import { getUserById, getUserData, setCandidatePhoneIndex, setUserData } from '../db.js';
...
import { canContinueWhatsAppAiAdvisor, getDisabledReason, getWhatsAppNumber } from './guard.js';
...
export async function start(candidateId, actorUser) {
  ...
  const whatsappNumber = getWhatsAppNumber(candidate);
...
export async function handleInbound(candidate, inboundMessage) {
  ...
  try {
    ...
    const sent = await sendViaWhatsApp(getWhatsAppNumber(updated), result.reply);
