...
export function shortHeader(value, fallback = 'Next step') {
  const clean = String(value || fallback).replace(/\s+/g, ' ').trim();
  return clean.length <= 28 ? clean : `${clean.slice(0, 27).trimEnd()}…`;
}

const TASK_AREAS = new Set(['Academics', 'Testing', 'Activities', 'University List', 'Essays', 'Applications', 'Admin']);
function taskArea(value) {
  if (TASK_AREAS.has(value)) return value;
  if (/test/i.test(value || '')) return 'Testing';
  if (/essay/i.test(value || '')) return 'Essays';
  if (/application/i.test(value || '')) return 'Applications';
  if (/school|university/i.test(value || '')) return 'University List';
  if (/admin/i.test(value || '')) return 'Admin';
  if (/project|research|lead|award|volunteer|activit/i.test(value || '')) return 'Activities';
  return 'Academics';
}
...
export function makeTask(input = {}, now = Date.now()) {
  ...
  return {
    ...
    header: shortHeader(input.header || input.title, 'Untitled task'),
    description: input.description || '',
    area: taskArea(input.area),
    owner: input.owner || 'candidate',
    priority: input.priority || 'medium',
    deadline: isoDay(input.deadline, now),
    status: input.status || 'todo',
...
// Testing status/target score/study plan/test dates for the Workplace
// Testing page. One plan per candidate (not a list), owned by the
// TestingAgent (lib/agents/sub/undergrad/TestingAgent.js).
export function makeTestingPlan(input = {}, now = Date.now()) {
  const created = ts(now);
  return {
    status: input.status || 'not_started', // not_started | in_progress | complete
    targetScore: input.targetScore || null,
    nextStep: input.nextStep || '',
    studyPlan: Array.isArray(input.studyPlan) ? input.studyPlan.filter(Boolean) : [],
    testDates: Array.isArray(input.testDates) ? input.testDates.map(d => isoDay(d, now)).filter(Boolean) : [],
    createdAt: input.createdAt || created,
    updatedAt: created,
  };
}

// One application per target school: deadlines, checklist, recommendation
// status, and overall submission status, for the Workplace Applications
// page. Owned by the ApplicationExecutionAgent
// (lib/agents/sub/undergrad/ApplicationExecutionAgent.js).
export function makeApplication(input = {}, now = Date.now()) {
  const created = ts(now);
  const id = input.id || makeId('appl', input.candidateId, input.schoolId || input.schoolName);
  return {
    id,
    candidateId: input.candidateId || null,
    schoolId: input.schoolId || null,
    schoolName: input.schoolName || 'Untitled school',
    deadlines: Array.isArray(input.deadlines)
      ? input.deadlines.map(d => ({ label: d.label || 'Deadline', date: isoDay(d.date, now) }))
      : [],
    checklist: Array.isArray(input.checklist)
      ? input.checklist.map(c => ({ label: c.label || 'Item', status: c.status || 'todo' }))
      : [],
    recommendations: Array.isArray(input.recommendations)
      ? input.recommendations.map(r => ({ name: r.name || 'Recommender', status: r.status || 'requested' }))
      : [],
    submissionStatus: input.submissionStatus || 'not_started', // not_started | in_progress | submitted
    createdAt: input.createdAt || created,
    updatedAt: created,
  };
}
