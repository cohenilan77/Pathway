...
export function runNagger(state, { now = Date.now(), candidateId = null, candidateName = '', candidateType = 'Undergraduate' } = {}) {
  ...
  const remind = (opts) => {
    ...
    if ((next.reminders || []).some(r => r.taskId === (opts.taskId || null) && r.title === opts.title && r.sentAt >= windowStart && r.status !== 'acknowledged')) return;
  ...
  const followUp = (opts) => {
    ...
    if ((next.tasks || []).some(t => t.source === 'nagger_follow_up' && t.title === task.title && t.status !== 'done')) return;
