...
import { isUndergradExploring, undergradProfileStage } from '../undergrad-profile.js';
...
export function journeyStages({ undergrad, scores = {}, profile = {}, programs = [], documents = [] } = {}) {
  ...
  return STAGE_DEFS.map(def => {
    ...
    if (def.key === 'programs') {
      ...
      const fields = ['grade', 'curriculum', 'subjects', 'countries', 'activities'];
      if (!isUndergradExploring(profile) && stage !== 'discovery') fields.push('intendedMajor');
