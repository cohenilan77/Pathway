...
import { cacheScholarshipResults } from '../../scholarships/scholarship-cache.js';
...
test('a school with a real cached scholarship gets a scholarshipNote and a chat callout', async () => {
  // MIT is in the curated pool (lib/undergrad/known-schools.js) and CS is
  // its strongest category, so a CS-major grade-10 profile should reliably
  // include it in the matched 12.
  await cacheScholarshipResults({
    schoolName: 'MIT',
    scholarships: [{ name: 'MIT Test Scholarship', url: 'https://example.com/mit-test', amountUSD: 20000 }],
  });
  const result = await handleUndergradSchoolListRequest('cand_scholarship_note', 'show me schools', {
    profile: { grade: '10', intendedMajor: 'Computer Science', gpa: 3.9, satScore: 1520 },
  });
  const mit = result.statePatch.programs.find(p => p.name === 'MIT');
  assert.ok(mit, 'MIT should be in the matched list for a strong CS profile');
  assert.match(mit.scholarshipNote, /MIT Test Scholarship/);
  assert.match(result.message, /💰.*MIT Test Scholarship/);
});

test('a school with nothing cached gets scholarshipNote: null, never a fabricated one', async () => {
  const result = await handleUndergradSchoolListRequest('cand_no_scholarship', 'show me schools', {
    profile: { grade: '10', intendedMajor: 'Humanities', gpa: 3.5 },
  });
  assert.ok(result.statePatch.programs.every(p => p.scholarshipNote === null || typeof p.scholarshipNote === 'string'));
});
...
test('coordinator integration: ordinary conversation mentioning school/program does not hijack into the hotfix handler', async () => {
  const originalHandle = UndergradAgent.prototype.handle;
  const calls = [];
  UndergradAgent.prototype.handle = async function stub(candidateId, message) {
    calls.push(message);
    return { agent: 'UndergradAgent', message: '', statePatch: {}, metadata: {} };
  };
  try {
    const coordinator = new HybridCoordinator();
    for (const message of ['tell me about my program requirements', 'can you see my school schedule tomorrow']) {
      const result = await coordinator.execute({
        candidateId: 'cand_no_hijack',
        message,
        candidateState: { profile: { category: 'Undergraduate', grade: '11', intendedMajor: 'Engineering' } },
      });
      assert.notEqual(result.agent, 'UndergradSchoolList');
    }
    assert.deepEqual(calls, ['tell me about my program requirements', 'can you see my school schedule tomorrow']);
  } finally {
