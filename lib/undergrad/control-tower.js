...
// Single-candidate command center (scoped Control Tower view). Aggregates the
// most important objects for ONE candidate: risk, progress, next action, urgent
// tasks, last activity, and the raw task/calendar/reminder objects the UI needs
// to render a real calendar. Pure — reads whatever engine state the candidate has.
export function buildCandidateCommandCenter(candidate, { now = Date.now() } = {}) {
  const rec = candidateEngine(candidate);
  const overdue = detectOverdueTasks(rec, now);
  const upcoming = detectUpcomingDeadlines(rec, now, 14);
  const openTasks = (rec.tasks || []).filter(t => !['done', 'cancelled'].includes(t.status));
  const nd = nextDeadline(rec, now);
  ...
  return {
    ...
    riskLevel: riskLevel(rec, now),
    lastActiveAt: rec.lastActiveAt,
    overallScore: Number.isFinite(Number(rec.scores?.overall)) ? Number(rec.scores.overall) : null,
    roadmapProgress: roadmapProgressPct(rec),
    roadmapTotal: (rec.roadmap || []).length,
    openTaskCount: openTasks.length,
    overdueCount: overdue.length,
    urgentCount: upcoming.length,
    nextDeadline: nd ? { title: nd.title, date: nd.date, type: nd.type } : null,
    nextAction: rec.nextAction || recommendedActionFor(rec, now),
    weakestArea: weakestArea(rec),
    tasks: rec.tasks || [],
    calendar: rec.calendar || [],
    reminders: (rec.reminders || []).filter(r => r.status !== 'acknowledged'),
    scores: rec.scores || {},
  };
}
