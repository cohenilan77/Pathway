...
export async function createManagedUser({ name, residency = '', email, username = '', age = null, password, role = ROLES.candidate, consultantId = '', plan = 'free', allowUnassigned = false }) {
  ...
  const user = {
    ...
    whatsappHumanChatActive: false,
    whatsappHumanChatPending: false,
    whatsappHumanChatPendingAt: null,
    whatsappHumanChatPendingMessages: [],
    whatsappHumanChatLastDeliveredAt: null,
    bsuid: '',
    lastActiveAt: Date.now(),
    createdAt: Date.now(),
    // Login tracking (for Telegram routing)
    lastLoginAt: null,
    lastLogoutAt: null,
...
export async function findOrCreateOAuthUser({ email, name, provider }) {
  ...
  if (existingId) {
    ...
    if (user) {
      ...
      const isDetailsComplete = hasCandidateDetails(user);
      const updated = {
        ...user,
        role: ROLES.candidate,
        plan: normalizePlan(user.plan),
        oauthProvider: user.oauthProvider || provider,
        oauthDetailsConfirmed: user.oauthDetailsConfirmed || isDetailsComplete,
        firstLoginSetupComplete: user.firstLoginSetupComplete !== undefined ? user.firstLoginSetupComplete : isDetailsComplete,
      };
  ...
  const user = {
    ...
    oauthDetailsConfirmed: false,
    firstLoginSetupComplete: false,
    plan: 'free',
    whatsappNumber: '',
    whatsappOptIn: false,
    whatsappOptInTimestamp: null,
...
export async function updateUserDetails(userId, details) {
  ...
  const updated = {
    ...
    firstLoginSetupComplete: details.firstLoginSetupComplete === true ? true : user.firstLoginSetupComplete,
    updatedAt: Date.now(),
  };
...
export async function destroySessionToken(token) {
  if (!token) return null;
  const store = getStore();
  const userId = await store.get(`session:${token}`);
  if (!userId) return null;
  await store.del(`session:${token}`);
  await store.srem(`sessions:byUser:${userId}`, token);
  const user = await store.get(`user:${userId}`);
  if (user) await store.set(`user:${userId}`, { ...user, lastActiveAt: 0 });
  return userId;
}
...
export function publicUser(user) {
  ...
  return {
    ...
    whatsappAiAdvisorTemplateConfigured: !!process.env.TWILIO_WHATSAPP_ADVISOR_KICKOFF_CONTENT_SID,
  };
}
...
// Login tracking for Telegram routing
export async function markCandidateLoggedIn(userId) {
  const store = getStore();
  const user = await store.get(`user:${userId}`);
  if (!user) throw new Error('User not found.');
  return await store.set(`user:${userId}`, {
    ...user,
...
export async function markCandidateLoggedOut(userId) {
  const store = getStore();
  const user = await store.get(`user:${userId}`);
  if (!user) throw new Error('User not found.');
  return await store.set(`user:${userId}`, {
...
export async function getCandidateLoginStatus(userId) {
  const store = getStore();
  const user = await store.get(`user:${userId}`);
  if (!user) return false;
  const lastActiveAt = Number(user.lastActiveAt || 0);
  return lastActiveAt > 0 && Date.now() - lastActiveAt < 2 * 60 * 1000;
}

// Telegram settings indexing
export async function getTelegramUserIdMapping(telegramUserId) {
  const store = getStore();
...
export async function setCandidateTelegramIdIndex(userId, telegramUserId) {
  const store = getStore();
  if (telegramUserId) {
    await store.set(`telegram:user:${String(telegramUserId).trim()}`, userId);
  }
}
