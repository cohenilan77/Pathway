...
export async function createManagedUser({ name, residency = '', email, username = '', age = null, password, role = ROLES.candidate, consultantId = '', plan = 'free', allowUnassigned = false }) {
  ...
  const user = {
    ...
    telegramAiAdvisorLastTurnAt: null,
    bsuid: '',
...
export async function getTelegramChatId(chatId) {
  const store = getStore();
  return await store.get(`telegram:chatid:${chatId}`);
}

export async function setTelegramChatIdIndex(candidateId, chatId) {
  const store = getStore();
  if (chatId) {
    await store.set(`telegram:chatid:${chatId}`, candidateId);
  }
}
