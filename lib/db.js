...
export async function createManagedUser({ name, residency = '', email, username = '', age = null, password, role = ROLES.candidate, consultantId = '', allowUnassigned = false }) {
...
export async function updateManagedUser(userId, patch) {
  ...
  const updated = {
    ...
    plan: patch.plan != null ? patch.plan : (user.plan || 'free'),
