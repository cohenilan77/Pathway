...
function assertSuperAdminEmailConfigured() {
  if (!SUPER_ADMIN_EMAIL && process.env.VERCEL_ENV === 'production') {
    throw new Error('SUPER_ADMIN_EMAIL environment variable must be set in production.');
  }
}
...
function isSuperAdminUser(user) {
  assertSuperAdminEmailConfigured();
...
export async function checkLoginRateLimit(ip) {
  ...
  if (existing == null) {
    await store.set(key, 1, { ex: LOGIN_RATE_LIMIT_WINDOW_SECONDS });
    return true;
  }
  const count = Number(existing) || 0;
  if (count >= LOGIN_RATE_LIMIT_MAX) return false;
  await store.set(key, count + 1, { ex: LOGIN_RATE_LIMIT_WINDOW_SECONDS });
  return true;
}
