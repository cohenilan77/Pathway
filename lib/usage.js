...
// Estimates how much of a record's input context could have been collapsed via
// headroom-based context compaction (older turns summarized once the context
// window fills up). Larger input contexts carry more redundant history, so the
// compressible share grows with input size, capped at a realistic ceiling.
export function estimateCompressionForRecord(record) {
  ...
  if (inputTokens <= 0) {
    return { compressionPct: 0, tokensSaved: 0, costSaved: 0 };
  }
  ...
  return { compressionPct, tokensSaved, costSaved };
}
