...
export function buildDeterministicAdvisorContext(candidateState = {}) {
  ...
  // readyForScoring alone gates scoring — a high evidence-confidence score
  // must never bypass a candidateFacts-level readiness gap (the previous
  // `confidence !== 'high' && !readyForScoring` check let exactly that slip
  // through).
...
// A candidate's standardized test score is an objective, source-backed fact.
// If the profile already carries any test signal, keep it; otherwise fill it
// from the deterministic resume/chat extraction so an omitted <PROFILE> echo
// never costs the candidate a score they actually reported. Never bridges soft
// evidence — only the single strongest reported test.
export function mergeSourcedTestScore(profile = {}, candidateFacts = {}) {
  const hasTestSignal = profile.gmat != null || profile.gre != null || profile.sat != null
    || profile.act != null || profile.testScore != null || !!profile.testType
    || profile.tests?.gmat != null || profile.tests?.gre != null
    || profile.standardizedTest?.score != null || profile.testOptional === true;
  if (hasTestSignal) return profile;
  const sourced = {};
  if (candidateFacts.gmat != null) sourced.gmat = candidateFacts.gmat;
  else if (candidateFacts.gre != null) sourced.gre = candidateFacts.gre;
  else if (candidateFacts.sat != null) sourced.sat = candidateFacts.sat;
  else if (candidateFacts.act != null) sourced.act = candidateFacts.act;
  return Object.keys(sourced).length ? { ...profile, ...sourced } : profile;
}
...
export function applyDeterministicKpiToResponse(response, { candidateState = {} } = {}) {
  ...
  const baseProfile = { ...(mergedProfile.candidateFacts || {}), ...mergedProfile };
  ...
  const candidateFacts = buildCandidateFacts({
    ...
    candidateType: baseProfile.category || baseProfile.degree,
    targetSchools: originalPatch.chosenSchools || candidateState.chosenSchools || [],
  });
  // Bridge the objective, source-extracted standardized test score onto the
  // profile so scoring credits a score the candidate actually reported (e.g. a
  // GMAT/GRE in their uploaded resume) even when the model failed to echo it
  // into its <PROFILE> patch. Only fills when the profile carries no test
  // signal of its own; soft evidence (leadership, narrative, recommenders) is
  // deliberately NOT bridged — those stay gated for explicit confirmation.
  const profile = mergeSourcedTestScore(baseProfile, candidateFacts);
  ...
  // readyForScoring is now a hard blocker on its own: a high evidence-density
  // score (confidence 'high') must never finalize while candidateFacts still
  // has a genuine readiness gap — that was the loophole that let weak/missing
  // KPIs slip through as final results instead of prompting a follow-up.
  if (evidenceCount(facts) < 2 || !candidateFacts.readyForScoring || (confidence.confidence !== 'high' && !bestAvailableRequested)) {
    const missingFields = [...new Set([...facts.evidence.missingFields, ...confidence.missingFields])];
    const pendingKpiResult = scoreCandidateKPIs(facts);
    ...
    const insights = {
      ...
      scoreDetails: pendingKpiResult.scoreDetails,
      missingFields,
      missingQuestions: pendingKpiResult.missingQuestions,
    ...
    // Ask the one consolidated missing-fields question (never one field at a
    // time) so this function is self-sufficient even for callers that don't
    // also run api/advisor's own candidateFacts gate.
    const question = buildComplementaryQuestion(candidateFacts);
    return {
      ...response,
      raw: question || raw,
      message: question || response.message,
      statePatch,
      metadata: {
        ...
        reason: 'missing_kpi_facts', missingFields, missingQuestions: pendingKpiResult.missingQuestions,
      },
    };
  }
  ...
  // Some KPIs may still come back missing or low-and-weak (LOW_SCORE_THRESHOLD
  // in kpi-engine.js). Ask about those once, combined — unless the candidate
  // was already asked, in which case accept the current value as final rather
  // than looping forever.
  const unresolvedConfirmations = (kpiResult.confirmationsNeeded || []).filter(key => !askedFields.has(key));
  if (unresolvedConfirmations.length > 0) {
    const confirmationBullets = unresolvedConfirmations
      .map(key => `• ${kpiResult.scoreDetails[key]?.displayReason || `More detail is needed on ${key} — your advisor will follow up.`}`)
      .join('\n');
    const question = `A few of your calculated results need a quick confirmation before I finalize your score:\n${confirmationBullets}`;
    const previousStrengths = candidateState.strengths?.length ? candidateState.strengths : originalPatch.strengths;
    const previousWeaknesses = candidateState.weaknesses?.length ? candidateState.weaknesses : originalPatch.weaknesses;
    const previousTasks = candidateState.tasks?.length ? candidateState.tasks : originalPatch.tasks;
    const insights = {
      ...currentInsights,
      deterministicKpiEngineFallback: true,
      confirmationsNeeded: unresolvedConfirmations,
      deterministicKpiEngine: { enabled: true, version: KPI_VERSION, confidence: confidence.confidence, fallback: true, reason: 'needs_confirmation' },
    };
    const updatedProfile = {
      ...profile,
      profileCompleteness: {
        ...completeness,
        askedFields: [...new Set([...(completeness.askedFields || []), ...unresolvedConfirmations])],
      },
    };
    const statePatch = {
      ...originalPatch,
    ...
    return {
      ...
      metadata: {
        ...(response.metadata || {}), deterministicKpiEngine: true, kpiVersion: KPI_VERSION,
        confidence: confidence.confidence, deterministicKpiEngineFallback: true, fallbackUsed: true,
        reason: 'needs_confirmation', confirmationsNeeded: unresolvedConfirmations,
      },
    };
  }
  ...
  if (Array.isArray(programs)) {
    const calibration = { kpiResult, scores: kpiResult.scores, overallScore: kpiResult.overall };
    programs = normalizeProgramList(programs.map(program => scoreProgramFit({ facts, kpiResult, program })), calibration) || programs;
    if (facts.track === 'MBA') programs = normalizeProgramList(balanceMbaPortfolio(programs), calibration) || programs;
  }
  const insights = {
    ...currentInsights,
    scoreDetails: kpiResult.scoreDetails,
    missingFields: kpiResult.missingFields,
    missingQuestions: kpiResult.missingQuestions,
