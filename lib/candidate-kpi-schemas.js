export const CANDIDATE_KPI_SCHEMAS = {
  Undergraduate: {
    ...
    requiredFacts: ['grade', 'curriculum', 'academic', 'subjects', 'activities'],
    optionalFacts: ['intendedMajor', 'interestCluster', 'testScore', 'leadershipEvidence', 'community', 'awards', 'narrative', 'targetSchools'],
    extractionFields: ['grade', 'profileStage', 'pathwayType', 'curriculum', 'gpa', 'grades', 'subjects', 'interests', 'interestCluster', 'activities', 'strongestActivity', 'leadershipEvidence', 'awards', 'tests', 'intendedMajor', 'countries', 'universityStyle', 'monthlySnapshots', 'kpiHistory', 'targetSchools'],
...
export function getCandidateKpiDisplayItems(scores = {}, profile = {}, candidateType, scoreDetails = {}) {
  ...
  return (schema?.kpis || []).map(([key, label, weight]) => {
    ...
    return {
      ...
      // Candidate-facing copy only — never the internal `reason` field
      // (logging/debug), which can contain internal scorer jargon.
      reason: value == null ? (detail.displayReason || `More detail is needed on ${label} — your advisor will follow up.`) : undefined,
      missingPrompt: value == null ? detail.missingPrompt : undefined,
    };
  });
}
