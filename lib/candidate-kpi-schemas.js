export const CANDIDATE_KPI_SCHEMAS = {
  ...
  MBA: {
    ...
    requiredFacts: ['workYears', 'leadershipEvidence', 'careerProgression', 'achievementsImpact', 'testScore', 'academic', 'whyMBA', 'postMbaGoal'],
    optionalFacts: ['recommenders', 'internationalExposure', 'community', 'targetSchools'],
...
export function resolveCandidateSchemaKey(profile = {}, candidateType) {
  const raw = String(candidateType || profile.category || profile.degree || profile.program || '').toLowerCase();
  if (/undergrad|bachelor|high school/.test(raw)) return 'Undergraduate';
  if (/\bmba\b/.test(raw)) return 'MBA';
  if (/phd|doctoral|doctorate|postgraduate/.test(raw)) return 'Postgraduate / Doctoral';
  if (/personal development|career development/.test(raw)) return 'Personal Development';
  return 'Graduate';
}
