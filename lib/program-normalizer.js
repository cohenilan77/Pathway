...
export function inferSelectivity(program = {}) {
  const existingLabel = validSelectivityLabel(program.selectivityLabel);
  if (existingLabel) {
    return {
      selectivityLabel: existingLabel,
      selectivitySource: program.selectivitySource || 'provided',
      selectivityScore: asNumber(program.selectivityScore) ?? SELECTIVITY_RANK[existingLabel] * 20,
    };
  }
