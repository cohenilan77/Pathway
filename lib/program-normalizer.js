...
function admissionStatusFromFit(fit, locked = false) {
  if (locked) return 'Not Eligible';
  const numericFit = asNumber(fit);
  if (numericFit != null && numericFit > 80) return 'Strong';
  if (numericFit != null && numericFit >= 50) return 'Competitive';
  return 'Reach';
}
...
function incompleteKpiCount(calibration = {}) {
  const scoreDetails = calibration.scoreDetails || calibration.kpiResult?.scoreDetails || calibration.insights?.scoreDetails || {};
  return Object.values(scoreDetails).filter(detail => detail?.incomplete || detail?.status === 'incomplete').length;
}

function overallScore(calibration = {}) {
  return asNumber(calibration.overallScore ?? calibration.kpiResult?.overall ?? calibration.scores?.overall);
}

function calibrateSchoolFit({ fit, locked, selectivityLabel, admitRate, calibration }) {
  let calibratedFit = clampFit(fit);
  if (calibratedFit == null || locked) return calibratedFit;

  const overall = overallScore(calibration);
  const incompleteCount = incompleteKpiCount(calibration);
  if (incompleteCount >= 4) calibratedFit -= 10;
  if (overall != null && overall < 70 && selectivityLabel === 'Ultra Competitive') calibratedFit -= 15;
  if (overall != null && overall < 70 && admitRate != null && admitRate < 10) calibratedFit = Math.min(calibratedFit, 49);
  if (overall != null && overall < 65 && admitRate != null && admitRate < 8) calibratedFit = Math.min(calibratedFit, 39);

  return clampFit(calibratedFit);
}
...
export function normalizeProgram(program = {}, calibration = {}) {
  const providedFit = asNumber(program.fit ?? program.fitIndex);
  ...
  const admissionStatus = admissionStatusFromFit(fit, locked);
  const evidenceGaps = Array.isArray(program.evidenceGaps) ? program.evidenceGaps.filter(Boolean) : [];
  const riskFlags = Array.isArray(program.riskFlags) ? program.riskFlags.filter(Boolean) : [];
  const missingActions = Array.isArray(program.missingActions)
    ? program.missingActions.filter(Boolean)
    : (Array.isArray(program.unlockConditions) && program.unlockConditions.length
      ? program.unlockConditions.filter(Boolean)
      : evidenceGaps.map(gap => `Resolve: ${gap}`));
  const tierMeta = {
    locked: { label: 'Locked', color: 'grey' },
    stretch: { label: 'Stretch', color: 'red' },
    possible: { label: 'Possible', color: 'yellow' },
    safe: { label: 'Strong Fit', color: 'green' },
  }[tier] || { label: 'Stretch', color: 'red' };
  const fitExplanation = String(program.fitExplanation || program.notes || program.programInfo || '').trim()
    || `${tierMeta.label} based on the candidate's evidenced profile and this program's requirements.`;
  return {
    ...program,
    fit: fit ?? program.fit,
    fitIndex: fit ?? program.fit,
    admissionStatus,
    tier,
    ...
    fitDrivers: Array.isArray(program.fitDrivers) ? program.fitDrivers.filter(Boolean) : [],
    riskFlags,
    evidenceGaps,
    missingActions,
    fitExplanation,
...
function normalizeCrossProgramFit(programs) {
  ...
  return programs.map((program) => {
    ...
    return {
      ...
      fitIndex: Math.max(asNumber(program.fit) ?? 0, 81),
      tier: 'safe',
...
export function normalizeProgramList(programs, calibration = {}) {
  return normalizeProgramListWithCalibration(programs, calibration);
}

export function normalizeProgramListWithCalibration(programs, calibration = {}) {
  ...
  for (const program of programs) {
    ...
    seen.add(key);
    normalized.push(normalizeProgram(program, calibration));
  }
