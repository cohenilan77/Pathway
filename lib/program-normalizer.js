...
function hasProgramSpecificMismatch(program = {}) {
  const text = [
    ...
    ...(Array.isArray(program.evidenceGaps) ? program.evidenceGaps : []),
    ...(Array.isArray(program.riskFlags) ? program.riskFlags : []),
    ...(Array.isArray(program.fitDrivers) ? program.fitDrivers : []),
  ].join(' ').toLowerCase();
  return /poor career|career-goal mismatch|goal mismatch|weak recruiting|limited recruiting|regional-only|regional placement|global outcome|missing prerequisite|prerequisite gap|format mismatch|part[- ]?time|online-only|research mismatch|supervisor mismatch|faculty mismatch|lab mismatch|portfolio mismatch|audition mismatch|different criteria|materially different criteria|not aligned|low alignment/.test(text);
}

function fitStatusFromFit(fit, locked = false) {
  const numericFit = asNumber(fit);
  if (locked || (numericFit != null && numericFit < 25)) return 'Below Baseline';
  if (numericFit == null) return undefined;
  if (numericFit >= 70) return 'Strong';
  if (numericFit >= 50) return 'Competitive';
  return 'Plausible';
}
...
export function normalizeProgram(program = {}) {
  ...
  return {
    ...
    hasProgramSpecificMismatch: hasProgramSpecificMismatch(program),
  };
}

function normalizeFitConsistency(programs) {
  const strongestByFamily = new Map();
  for (const program of programs) {
    if (program.tier === 'locked' || program.hasProgramSpecificMismatch) continue;
    const fit = asNumber(program.fit);
    if ((program.admissionStatus === 'Strong' || (fit != null && fit >= 70)) && fit != null && fit >= 70) {
      const family = programFamily(program);
      const current = strongestByFamily.get(family);
      if (!current || (asNumber(program.selectivityScore) ?? 0) > current.selectivityScore) {
        strongestByFamily.set(family, { selectivityScore: asNumber(program.selectivityScore) ?? 0, fit });
      }
    }
  }

  return programs.map((program) => {
    const reference = strongestByFamily.get(programFamily(program));
    const selectivityScore = asNumber(program.selectivityScore) ?? 0;
    const fit = asNumber(program.fit);
    if (!reference || program.tier === 'locked' || program.hasProgramSpecificMismatch || selectivityScore >= reference.selectivityScore || fit == null || fit > 80) {
      return program;
    }

    const normalizedFit = Math.max(fit, Math.min(82, reference.fit));
    return {
      ...program,
      fit: normalizedFit,
      tier: tierFromFit(normalizedFit, false),
      admissionStatus: fitStatusFromFit(normalizedFit),
      fitDrivers: Array.from(new Set([...(program.fitDrivers || []), 'Fit normalized against more selective same-category programs'])),
    };
  });
}
...
export function normalizeProgramList(programs) {
  ...
  const consistencyNormalized = normalizeFitConsistency(normalized);
