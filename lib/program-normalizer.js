...
export function normalizeProgram(program = {}) {
  ...
  // Resolve admit rate from admitRate or acceptanceRate field
  const admitRateRaw = program.admitRate ?? program.acceptanceRate ?? null;
  const admitRate = asNumber(admitRateRaw);
  return {
    ...program,
    fit: fit ?? program.fit,
    tier,
    ...
    admitRateSource: program.admitRateSource || (admitRate !== null ? 'AI estimate' : 'Not available'),
  };
}
