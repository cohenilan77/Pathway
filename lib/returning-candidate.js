...
export function buildReturningCandidateMessage({ user, profile, scores, programs, chosenSchools, narrative, cvText, essays, interviews, chat, undergrad } = {}) {
  ...
  // Undergraduate has its own coaching-tone welcome-back — never the grad
  // "generate your university list / program portfolio" pipeline prompts.
  if (category === 'Undergraduate') {
    ...
    // No saved session summary yet. A brand-new student (no real chat history)
    // goes to onboarding; a returning one gets a warm, low-pressure opener
    // pointed at building their profile — not a school-list push.
    if ((chat?.length || 0) <= 2) return null;
    return `Welcome back, ${name}! 👋 Good to see you again — want to take one more step on your profile? → What should I focus on? | Continue building my profile`;
  }
