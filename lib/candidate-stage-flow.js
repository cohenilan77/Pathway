...
// Tool-driven advance: once NarrativeCoachAgent's save_narrative_text tool
// locks the sharpened pitch (NARRATIVE_COACHING_V2), the stage advances
// straight to CV without waiting for an explicit "let's work on my CV" ask —
// every other stage's regex gate above is unchanged. Callers only invoke this
// when a narrative text was actually just saved this turn (see
// statePatch.narrativeText in src/App.jsx), so no extra "is it new" check is
// needed here.
export function advanceOnNarrativeLock(stepIdx, steps = null) {
  const CV = stepIndex(steps, 'CV');
  if (CV < 0) return null;
  return stepIdx < CV ? CV : null;
}
...
// PhD/Postgraduate-Doctoral candidates never see the MBA/Graduate Upgrade/
// Pivot Narrative & Strategy stage or an "Essays" stage — they get PHD STEP 5
// (Research Positioning) and PHD STEP 7 (Statement of Purpose / Research
// Statement) instead (see api/chat.js). Callers pass state.category through
// so this guardrail's own wording never contradicts that.
function isPhdCategory(category) {
  return String(category || '').trim() === 'Postgraduate / Doctoral';
}
...
export function requiredNextStage(state = {}) {
  ...
  const isPhd = isPhdCategory(state.category);
  ...
  if (!s.hasChosen) return { stage: 'Choose target schools', message: isPhd ? "First choose your target schools from the list, then we position your research." : "First choose your target schools from the list, then we shape your narrative." };
  if (!s.narrativeChosen) return isPhd ? { stage: 'Research Positioning', message: "Now let’s position your research." } : { stage: 'Narrative & Strategy', message: "Now let’s shape your Narrative & Strategy." };
  if (!s.cvUnlocked) return { stage: 'CV optimization', message: isPhd ? "With your research narrative underway, we can move on to optimizing your CV." : "With your narrative underway, we can move on to optimizing your CV." };
  return isPhd
    ? { stage: 'Statement of Purpose / Research Statement', message: "Your CV stage is unlocked, so your Statement of Purpose and Research Statement are next." }
    : { stage: 'Essays', message: "Your CV stage is unlocked, so essays are the next step." };
}
