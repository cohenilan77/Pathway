...
const SCHOOL_LIST_INTENT = /(?:show|generate|build|create|recommend|suggest|refresh)\s+(?:me\s+)?(?:my\s+)?(?:complete\s+|full\s+|recommended\s+|tailored\s+)?(?:school|program|university)?\s*(?:list|portfolio|matches|schools|programs|universities)|(?:cannot|can't|do not|don't)\s+see[\s\S]{0,80}(?:schools?|programs?|portfolio|list|matches)|\bmatch\s+me\b|^\s*(?:recommend|portfolio|school\s+list|university\s+list)\s*[.!]?\s*$/i;
...
export function buildCandidateFacts(input = {}) {
  ...
  const programTypeKnown = isKnown(explicitProgramType)
  ...
  const nextQuestionFields = criticalMissingFields.filter(field => !rememberedAsked.includes(field)).slice(0, 3);
  ...
  merged.readyForScoring = merged.selectedCandidateType === 'MBA'
    ? (standardReady
  ...
  merged.readyForPrograms = merged.readyForScoring && programTypeKnown && (recommendationPathKnown || specificPathReady);
