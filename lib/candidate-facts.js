...
function parseEducationFacts(text) {
  const education = unique(String(text || '').split(/\n+|(?<=[.!?])\s+/)
    .map(line => line.trim())
    .filter(line => /\b(bachelor(?:'s)?|master(?:'s)?|bsc|msc|b\.a\.|m\.a\.|undergraduate degree|graduate degree|graduated from|university|college)\b/i.test(line))
...
function fieldValue(facts, field) {
  const aliases = {
    academic: facts.academic || facts.gpa || facts.grades || facts.education || facts.degrees || facts.school || facts.university,
...
export function buildCandidateFacts(input = {}) {
  ...
  Object.assign(merged, parseEducationFacts(text), parseScoreFacts(text), parseNarrativeFacts(text), parseTargetFacts(text));
...
export function buildComplementaryQuestion(candidateFacts) {
  const unasked = candidateFacts?.nextMissingFields || [];
  const stillMissing = candidateFacts?.profileCompleteness?.missingFields || [];
  // Fields already asked but never answered stay missing; re-ask them rather
  // than dead-ending the conversation with a checklist the user can't see.
  const fields = unasked.length ? unasked : stillMissing;
  if (!fields.length) return '';
  const items = fields.map(field => QUESTION_LABELS[field] || field).join('; ');
  return unasked.length
    ? `Before I finalize your score, please fill in only these missing items: ${items}.`
    : `To finalize your score I still need: ${items}. Answer here in one message, or upload your CV and I'll extract what I can automatically.`;
}
