...
export function buildCandidateFacts(input = {}) {
  ...
  Object.assign(merged, parseEducationFacts(text), parseScoreFacts(text), parseNarrativeFacts(text), parseTargetFacts(text), parseProgramFacts(text));
  ...
  const advanceRequested = isAdvanceRequest(latestUserText);
  ...
  merged.readyForScoring = (missingFields.length === 0 && confidence >= 0.75)
    || ((advanceRequested || checklistExhausted) && evidenceCount >= 2);
...
export function buildComplementaryQuestion(candidateFacts) {
  if (candidateFacts?.readyForScoring) return '';
...
export function candidateFactsPrompt(candidateFacts) {
  ...
  return [
    ...
    'Do not emit SCORES until readyForScoring is true. Do not emit PROGRAMS until readyForPrograms is true. When advanceRequested is true, proceed with best available evidence and turn unknown fields into follow-up tasks instead of asking again.',
