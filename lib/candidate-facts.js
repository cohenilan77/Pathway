...
export function extractWorkTimeline(text, now = new Date()) {
  ...
  lines.forEach((line, index) => {
    for (const match of line.matchAll(range)) {
      ...
      const military = /\b(idf|army|military|armed forces?|navy|air force|defen[cs]e force|national service)\b/i.test(context);
      roles.push({
...
export function buildCandidateFacts(input = {}) {
  ...
  const merged = Object.assign({}, extractedObject, ...blocks, profile);
