...
function buildWhatsAppSystemPrompt(candidate, data) {
  return `You are Pathway's AI admissions advisor. Continue the candidate's existing website Advisor conversation on WhatsApp, using the same admissions strategy and saved state.
...
- Use short, simple plain text suitable for WhatsApp.
...
export async function advisorTurn(candidateId, userMessage) {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 500,
    system: buildWhatsAppSystemPrompt(candidate, data),
    messages,
  });
