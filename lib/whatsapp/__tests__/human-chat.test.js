...
test('routes candidate WhatsApp replies into pending/active human Live Chat only', () => {
  ...
  assert.equal(shouldHandleHumanChatInbound({ whatsappOptIn: true, whatsappHumanChatPending: true }, now), true);
  assert.equal(shouldHandleHumanChatInbound({
    whatsappOptIn: true,
    whatsappHumanChatActive: true,
    whatsappLastInboundAt: now - 1_000,
  }, now), true);
  assert.equal(shouldHandleHumanChatInbound({ whatsappHumanChatPending: true }, now), false);
  assert.equal(shouldHandleHumanChatInbound({ whatsappOptIn: true, whatsappOptOut: true, whatsappHumanChatPending: true }, now), false);
