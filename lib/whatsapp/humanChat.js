import { appendMessage } from '../chat.js';
import { getStore } from '../store.js';
import { sendViaWhatsApp } from './outbound.js';
import { normalizeWhatsAppNumber } from './phone.js';

const HUMAN_CHAT_WINDOW_MS = 24 * 60 * 60 * 1000;
const MAX_PENDING_MESSAGES = 10;
const MAX_WHATSAPP_TEXT = 1500;

export function isHumanChatStaff(sender) {
  return sender === 'consultant' || sender === 'admin';
}

export function shouldHandleHumanChatInbound(candidate, now = Date.now()) {
  if (candidate?.whatsappOptIn !== true || candidate?.whatsappOptOut === true) return false;
  if (candidate?.whatsappHumanChatPending === true) return true;
  const lastInbound = Number(candidate?.whatsappLastInboundAt || 0);
  return candidate?.whatsappHumanChatActive === true
    && lastInbound > 0
    && now - lastInbound < HUMAN_CHAT_WINDOW_MS;
}

export function buildHumanChatTemplateVariables(candidate) {
  return { '1': String(candidate?.name || 'there').trim() || 'there' };
}

export function buildPendingHumanChatReply(messages) {
  const text = (messages || [])
    .map((message) => String(message?.text || '').trim())
    .filter(Boolean)
    .join('\n\n');
  if (!text) return '';
  return text.length <= MAX_WHATSAPP_TEXT ? text : `${text.slice(0, MAX_WHATSAPP_TEXT - 1).trim()}…`;
}

async function updateCandidate(candidateId, patch) {
  const store = getStore();
  const candidate = await store.get(`user:${candidateId}`);
  if (!candidate) throw new Error('Candidate not found.');
  const updated = { ...candidate, ...patch };
  await store.set(`user:${candidateId}`, updated);
  return updated;
}

export async function markHumanChatPending(candidateId, { sender, text, at = Date.now() }) {
  const store = getStore();
  const candidate = await store.get(`user:${candidateId}`);
  if (!candidate) throw new Error('Candidate not found.');
  const pending = Array.isArray(candidate.whatsappHumanChatPendingMessages)
    ? candidate.whatsappHumanChatPendingMessages.slice(-(MAX_PENDING_MESSAGES - 1))
    : [];
  pending.push({ sender, text: String(text || '').trim(), at });
  return updateCandidate(candidateId, {
    whatsappHumanChatPending: true,
    whatsappHumanChatPendingAt: candidate.whatsappHumanChatPendingAt || at,
    whatsappHumanChatPendingMessages: pending,
  });
}

export async function markHumanChatActive(candidateId, lastInboundAt) {
  return updateCandidate(candidateId, {
    whatsappHumanChatActive: true,
    whatsappHumanChatPending: false,
    whatsappHumanChatPendingAt: null,
    whatsappHumanChatPendingMessages: [],
    whatsappLastInboundAt: lastInboundAt,
  });
}

export async function handleHumanChatInbound(candidate, inboundMessage) {
  const store = getStore();
  const sourceMessageId = inboundMessage.sourceMessageId || null;
  const dedupeKey = sourceMessageId ? `whatsapp:human-inbound:${sourceMessageId}` : null;
  if (dedupeKey && await store.get(dedupeKey)) {
    return { duplicate: true, replied: false };
  }

  const current = await store.get(`user:${candidate.id}`) || candidate;
  await appendMessage(candidate.id, {
    senderId: candidate.id,
    senderRole: 'candidate',
    text: inboundMessage.text,
    sourceMessageId,
  });
  const now = Date.now();
  await updateCandidate(candidate.id, {
    whatsappHumanChatActive: true,
    whatsappLastInboundAt: now,
  });

  const pendingMessages = Array.isArray(current.whatsappHumanChatPendingMessages)
    ? current.whatsappHumanChatPendingMessages
    : [];
  const reply = buildPendingHumanChatReply(pendingMessages);
  if (reply) {
    const sent = await sendViaWhatsApp(
      normalizeWhatsAppNumber(current.whatsappNumber || current.phone),
      reply
    );
    if (!sent?.success) throw new Error(sent?.error || 'Could not deliver the pending Live Chat message.');
  }

  await updateCandidate(candidate.id, {
    whatsappHumanChatActive: true,
    whatsappHumanChatPending: false,
    whatsappHumanChatPendingAt: null,
    whatsappHumanChatPendingMessages: [],
    whatsappHumanChatLastDeliveredAt: reply ? Date.now() : current.whatsappHumanChatLastDeliveredAt || null,
  });
  if (dedupeKey) await store.set(dedupeKey, { status: 'done' }, { ex: 7 * 24 * 60 * 60 });
  return { duplicate: false, replied: !!reply, reply: reply || null };
}
