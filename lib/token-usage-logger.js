...
import { calculateUsageCost, getUsageResetAt, pricingFor } from './usage.js';

async function logsSinceReset(logs) {
  const resetAt = await getUsageResetAt();
  return (logs || []).filter((log) => (Number(log?.timestamp) || 0) >= resetAt);
}
...
export async function logTokenUsage({
  ...
  const costs = calculateUsageCost({ model, inputTokens, outputTokens, cacheCreationInputTokens, cacheReadInputTokens, webSearchRequests });
...
export async function getAllTokenUsageLogs() {
  ...
  return logsSinceReset(logs.filter(Boolean));
}
