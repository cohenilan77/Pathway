const TRACK_WEIGHTS = {
  Undergraduate: { academic: 18, testScore: 10, activities: 11, leadership: 10, volunteering: 7, awards: 8, narrative: 8, goalClarity: 8, potential: 12, uniqueness: 8 },
  MBA: { professional: 20, leadership: 18, careerProgression: 14, internationalExposure: 8, testScore: 13, academic: 12, narrative: 15 },
  Graduate: { academic: 22, professional: 14, leadership: 10, research: 14, testScore: 13, narrative: 14, goalClarity: 13 },
  'Postgraduate / Doctoral': { academic: 18, research: 22, publications: 14, facultyFit: 16, potential: 14, narrative: 8, recommenders: 8 },
  'Personal Development': { professional: 22, leadership: 18, goalClarity: 20, narrative: 18, potential: 22 },
};
...
function detail(score, reason, evidence, confidence = 'medium') {
  return { score: clamp(score), reason, evidence, confidence };
}
...
function academic(facts) {
  ...
  } else result = detail(50, 'Academic evidence is incomplete.', 'No verified GPA or transcript strength', 'low');
...
function testing(facts) {
  ...
  if (testScore == null) return testOptional
    ...
    : detail(40, 'A required or useful standardized test score is missing.', 'No test evidence', 'low');
  const thresholds = {
...
function professional(facts) {
  ...
  if (years == null) return detail(45, 'Relevant experience duration is not confirmed.', 'Missing work-years evidence', 'low');
...
function leadership(facts) {
  ...
  return detail(40, 'No clear leadership evidence is recorded.', 'Missing leadership evidence', 'low');
}

function progression(facts) {
  const value = text(facts.experience.careerProgression);
  if (/promotion|increasing|rapid|progress/.test(value)) return detail(85, 'Clear promotions or increasing responsibility are recorded.', 'Career progression', 'high');
  if (facts.experience.workYears != null) return detail(value ? 65 : 60, 'Work history is present, but promotion evidence is limited.', 'Work history', 'medium');
  return detail(50, 'Career progression is unclear.', 'Missing progression evidence', 'low');
}
...
function international(facts) {
  ...
  return detail(45, 'International exposure evidence is limited.', 'Missing international evidence', 'low');
}
...
function research(facts) {
  ...
  return detail(35, 'No research evidence is recorded.', 'Missing research evidence', 'low');
}
...
function publications(facts) {
  ...
  return detail(35, 'No publication or thesis evidence is recorded.', 'Missing scholarly output', 'low');
}
...
function facultyFit(facts) {
  ...
  return detail(35, 'Faculty or lab fit is not established.', 'Missing faculty-fit evidence', 'low');
}
...
function recommenders(facts) {
  ...
  return detail(40, 'No direct evaluator recommender is confirmed.', 'Missing recommender strategy', 'low');
}
...
function activities(facts) {
  ...
  return detail(35, 'No activity evidence is recorded.', 'Missing activity evidence', 'low');
}
...
function volunteering(facts) {
  ...
  return detail(35, 'No volunteering evidence is recorded.', 'Missing volunteering evidence', 'low');
}
...
function awards(facts) {
  ...
  return detail(35, 'No awards or recognition are recorded.', 'Missing awards evidence', 'low');
}
...
function narrative(facts) {
  ...
  return detail(40, 'Why degree, why now, and career goal are unclear.', 'Missing narrative evidence', 'low');
}

function goals(facts) {
  const goal = text(facts.narrative.careerGoal);
  if (!goal) return detail(35, 'No career or academic goal is confirmed.', 'Missing goal', 'low');
  const specific = /manager|consultant|analyst|engineer|researcher|professor|founder|director|doctor|lawyer|product|investment/.test(goal);
...
function uniqueness(facts) {
  ...
  return detail(45, 'No distinctive angle is yet evidenced.', 'Missing uniqueness evidence', 'low');
}

function weightedOverall(scores, track) {
  const weights = TRACK_WEIGHTS[track] || TRACK_WEIGHTS.Graduate;
  const entries = Object.entries(weights).filter(([key]) => Number.isFinite(scores[key]));
  const totalWeight = entries.reduce((sum, [, weight]) => sum + weight, 0);
  return clamp(entries.reduce((sum, [key, weight]) => sum + scores[key] * weight, 0) / (totalWeight || 1));
}

export function scoreCandidateKPIs(facts) {
  const scoreDetails = {
    academic: academic(facts), testScore: testing(facts), professional: professional(facts), leadership: leadership(facts),
    careerProgression: progression(facts), internationalExposure: international(facts), research: research(facts),
  ...
  const scores = Object.fromEntries(Object.entries(scoreDetails).map(([key, value]) => [key, value.score]));
  scoreDetails.potential = potential(facts, scores);
  scoreDetails.uniqueness = uniqueness(facts);
  scores.potential = scoreDetails.potential.score;
  scores.uniqueness = scoreDetails.uniqueness.score;
  ...
  const ranked = relevant.map(key => [key, scoreDetails[key]]).filter(([, value]) => value).sort((a, b) => b[1].score - a[1].score);
