...
function detail(score, reason, evidence, confidence = 'medium') {
  if (score == null) return { score: null, status: 'incomplete', reason, evidence, confidence: 'low', incomplete: true };
  const result = { score: clamp(score), status: 'scored', reason, evidence, confidence, incomplete: false };
...
function professional(facts) {
  ...
  return detail(Math.min(95, score), `${years} years of experience${has(facts.experience.achievements) ? ' with achievement evidence' : ''}${confirmedNote}`, 'Work history', confidence);
}
