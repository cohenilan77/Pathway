...
function progression(facts) {
  ...
  return detail(null, 'Career progression is unclear.', 'Missing progression evidence', 'low');
}
...
export function scoreCandidateKPIs(facts) {
  ...
  const scores = Object.fromEntries(Object.entries(scoreDetails).filter(([, value]) => Number.isFinite(value.score)).map(([key, value]) => [key, value.score]));
  scoreDetails.potential = potential(facts, scores);
  scoreDetails.uniqueness = uniqueness(facts);
  if (Number.isFinite(scoreDetails.potential.score)) scores.potential = scoreDetails.potential.score;
  if (Number.isFinite(scoreDetails.uniqueness.score)) scores.uniqueness = scoreDetails.uniqueness.score;
...
export function assessScoringConfidence(facts) {
  ...
  if (facts.track === 'MBA') {
    ...
    if (facts.academics.gpa == null && facts.experience.workYears == null) missing.push('academic or professional baseline');
