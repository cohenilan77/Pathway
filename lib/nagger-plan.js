import { computeRoadmapGaps } from './undergrad/roadmap-milestones.js';
...
export function buildNaggerPlan({ profile = {}, state = {}, events = [], now = Date.now() } = {}) {
  ...
  // Plan-aware nudge: a grade-11 candidate still missing a booked test date
  // heading into the June test cycle's registration window (roughly
  // Jan-May) gets a specific, time-critical nudge instead of the generic
  // undergradFocus() fallback below. computeRoadmapGaps reads the same
  // hidden per-grade milestone plan lib/agents/UndergradAgent.js steers by.
  if (undergrad && grade === 11) {
    const roadmapGaps = computeRoadmapGaps({
      profile: { ...profile, ...state.profile },
      undergrad: state.undergrad,
      programs: state.programs,
      documents: state.documents,
    });
    if (roadmapGaps.includes('test date booked') && new Date(now).getUTCMonth() <= 4) {
      staleItems.unshift('SAT/ACT June registration closes soon — lock in a test date.');
    }
  }
