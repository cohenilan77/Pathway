import { tierFromFit } from './program-normalizer.js';
...
function averageSoftScores(softScores, track) {
  const keys = track === 'undergraduate'
    ? ['activities', 'leadership', 'volunteering', 'awards', 'uniqueness', 'goalClarity', 'potential']
    : track === 'phd'
      ? ['research', 'publications', 'facultyFit', 'potential', 'recommenders', 'goalClarity']
      : track === 'mba'
        ? ['professional', 'leadership', 'careerProgression', 'internationalExposure', 'goalClarity']
        : ['professional', 'leadership', 'research', 'volunteering', 'uniqueness', 'goalClarity'];
  const vals = keys
    .map((k) => softScores?.[k])
...
export function computeFit(candidate, program) {
  ...
  const avgSoft = averageSoftScores(candidate.softScores, candidate.track);
  let fit = (gpaGapScore(gpaGap) + testGapScore(testGap)) / 2 + softBooster(avgSoft);

  // Selectivity belongs on program.selectivityLabel, not in the row/group fit tier.
  ...
  if (isSevereGap && exceptionType === 'partial') fit = Math.min(fit, 49);
  if (exceptionFlag) fit = Math.max(fit, 18);
  fit = Math.max(5, Math.min(95, Math.round(fit)));

  return { fit, tier: tierFromFit(fit, false), unlockConditions: [], exceptionFlag };
}
