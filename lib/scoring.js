...
export function computeFit(candidate, program) {
  ...
  const isDoubleGap = gpaGap < -LOCKED_GPA_GAP && testGap < -LOCKED_TEST_GAP;

  if (isDoubleGap && exceptionType !== 'true') {
  ...
  const exceptionFlag = exceptionType === 'true' && isDoubleGap;
  const tier = isDoubleGap && exceptionType === 'partial' ? 'stretch' : tierFromFit(fit);
