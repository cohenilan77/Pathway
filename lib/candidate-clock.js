...
// Undergraduate nudge cadence scales with grade urgency: Grade 9-10 is light/quarterly
// (there's no imminent deadline), Grade 11-12 is frequent/deadline-driven.
const UNDERGRAD_NUDGE_INTERVALS = {
  light: 90 * 24 * 60 * 60 * 1000, // ~quarterly, Grade 9-10
  frequent: 14 * 24 * 60 * 60 * 1000, // biweekly, Grade 11-12
};
const SEMESTER_MS = 182 * 24 * 60 * 60 * 1000;

export function getUndergradNudgeIntervalMs(gradeNumber) {
  if (gradeNumber != null && gradeNumber >= 11) return UNDERGRAD_NUDGE_INTERVALS.frequent;
  return UNDERGRAD_NUDGE_INTERVALS.light;
}
...
export async function initializeCandidateClock(userId, journeyType) {
  ...
  const clock = {
    ...
    lastTranscriptNudgeAt: null,
    nextTranscriptNudgeAt: journeyType === 'undergraduate' ? now + SEMESTER_MS : null,
    timezone: 'UTC',
...
export async function markWeeklyCheckInCompleted(userId, intervalMs = 7 * 24 * 60 * 60 * 1000) {
  const now = Date.now();
  return updateCandidateClock(userId, {
    lastWeeklyCheckInAt: now,
    nextWeeklyCheckInAt: now + intervalMs,
  });
}

export async function markMonthlyReviewCompleted(userId, intervalMs = 30 * 24 * 60 * 60 * 1000) {
  const now = Date.now();
  return updateCandidateClock(userId, {
    lastMonthlyReviewAt: now,
    nextMonthlyReviewAt: now + intervalMs,
  });
}

export async function isTranscriptNudgeDue(userId) {
  const clock = await getCandidateClock(userId);
  if (!clock?.nextTranscriptNudgeAt) return false;
  return Date.now() >= clock.nextTranscriptNudgeAt;
}

export async function markTranscriptNudgeSent(userId) {
  const now = Date.now();
  return updateCandidateClock(userId, {
    lastTranscriptNudgeAt: now,
    nextTranscriptNudgeAt: now + SEMESTER_MS,
  });
}
