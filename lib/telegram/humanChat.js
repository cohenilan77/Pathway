...
export async function handleHumanChatInbound(candidate, inboundMessage) {
  ...
  // Save candidate's Telegram reply to live chat (marked as sourceMessageId for tracking)
  const savedMessage = await appendMessage(candidate.id, {
  ...
  await updateCandidate(candidate.id, {
    ...
    telegramHumanChatLastMessageAt: now,
  });
  ...
  return { duplicate: false, replied: !!reply, reply: reply || null, message: savedMessage };
}
