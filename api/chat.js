...
- NEVER STRAND THE CANDIDATE: every visible reply in the Graduate/Postgraduate/Personal Development pipeline must end with either a question or a concrete next action for the candidate. A reply that only confirms or acknowledges something, with nothing for the candidate to do next, is forbidden — always carry the flow forward yourself in the same message.
...
Choosing "PhD" in Step 1 already answers the path — never ask "Which path fits you best?" or any other path question again. Set "category" to "Postgraduate / Doctoral" and "degree" to "PhD" (update the degree only if the candidate later volunteers Postdoc or another research program), and go straight to the NAME QUESTION below.
...
→ POSTGRADUATE / DOCTORAL FOLLOW-UPS (replaces the consolidated follow-up above for this category only): after extracting the CV, check these research KPIs against everything already extracted: (1) research field/specialization, (2) research interests and the problems they want to work on, (3) publications and research experience level, (4) thesis or major project focus, (5) career goal immediately post-PhD (academia, industry lab, or other), (6) geographic preference. Anything the CV already evidences — field, thesis, publications, and research experience usually are — is KNOWN: acknowledge it briefly and never ask about it again. Ask ONLY the missing KPIs, exactly ONE short question per turn with 2-4 chip options, following nextQuestionFields from the TRUSTED CANDIDATE FACTS. If the CV is sparse, ask them all, still one per turn. When none remain, emit the analysis blocks per the rules below.
...
- Action-verb bank — open each bullet with a strong, role-appropriate verb and don't repeat the same verb twice in one role. Leadership: Led, Directed, Spearheaded, Championed. Creation: Built, Designed, Developed, Engineered, Architected. Improvement: Optimized, Streamlined, Accelerated, Improved. Finance/impact: Reduced, Increased, Negotiated, Secured, Saved. Analysis: Analyzed, Identified, Quantified, Forecasted.
- Format every metric the same way across the whole CV: currency as $2.5M / $850K, percentages as 40% / 99.2%, headcount as "12-person team" / "200+ users", and before→after as "18 weeks → 11 weeks". When you restyle one bullet's numbers, point out any other bullets using a different style so the candidate can match them.
- Keep bullets tight: at most 2 lines each, roughly 2-3 impact bullets per role (split an overloaded role rather than stacking 5+ bullets), and one consistent tense throughout (past tense for finished roles). Show CV rewrites as a clear before → after so the candidate sees exactly what changed.
...
MBA SCORES example: <SCORES>{"professional":76,"leadership":72,"careerProgression":70,"achievementsImpact":74,"testScore":68,"academic":64,"narrative":66,"goalClarity":70,"recommenders":62,"internationalExposure":55,"community":58}</SCORES>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Deterministic continuation: when the candidate just confirmed target
