...
const btnPrimary = {
  background: 'linear-gradient(135deg,#1d3b32,#1d3b32)', color: '#faf7f2', border: 'none', borderRadius: 12,
  fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 8px 16px rgba(105,91,255,.3)',
};
const btnGhost = {
  background: '#faf7f2', color: '#2c3833', border: '1px solid #f1eadd', borderRadius: 10,
  fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
};
const btnDanger = {
  background: '#f3e3df', color: '#a8453f', border: '1px solid #f3e3df', borderRadius: 10,
  fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
};

const scoreColor = (s) => (s >= 75 ? '#2f6b4f' : s >= 50 ? '#c0844a' : '#a8453f');
const tierColor = (tier) => (tier === 'stretch' ? '#a8453f' : tier === 'safe' ? '#2f6b4f' : tier === 'possible' ? '#c0844a' : '#5d6b63');
const tierBg = (tier) => (tier === 'stretch' ? '#f3e3df' : tier === 'safe' ? '#cdd8d1' : tier === 'possible' ? '#e9c79a' : '#f1eadd');
const tierBorder = (tier) => (tier === 'stretch' ? '#f3e3df' : tier === 'safe' ? '#bcccc4' : tier === 'possible' ? '#e9c79a' : '#f1eadd');
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f1eadd', fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ width: 258, flexShrink: 0, background: '#faf7f2', borderRight: '1px solid #f1eadd', display: 'flex', flexDirection: 'column', padding: '26px 18px', minHeight: '100vh' }}>
        <div style={{ padding: '0 8px 8px' }}>
          <div style={{ fontSize: 23, fontWeight: 800, color: '#1a2420' }}>Pathway</div>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1px', color: '#1d3b32', marginTop: 2 }}>ADMIN PORTAL</div>
        </div>
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 8, background: '#f6f1e8', borderRadius: 14 }}>
            <span style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg,#122621,#122621)', color: '#c0844a', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 16 }}>✦</span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#1a2420' }}>Admin Panel</div>
              <div style={{ fontSize: 11, color: '#5d6b63', letterSpacing: '.5px' }}>IVY ADMISSIONS</div>
            </div>
          </div>
          <button onClick={signOut} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, color: '#5d6b63', fontWeight: 600, padding: 8, width: '100%', marginTop: 8 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, padding: '26px 36px', borderBottom: '1px solid #f1eadd', background: '#faf7f2' }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: '#1a2420', margin: 0 }}>
          ...
          {sessionActive && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#cdd8d1', border: '1px solid #bcccc4', borderRadius: 10, padding: '8px 16px' }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#2f6b4f' }} />
              <span style={{ fontSize: 13, fontWeight: 700, color: '#2f6b4f' }}>Session Active</span>
            </div>
          )}
        </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '30px 36px' }}>
          ...
          {adminView === 'candidates' && !candidateOpen && (
            <div>
              ...
              {usersError ? (
                <div style={{ background: '#f3e3df', border: '1px solid #f3e3df', borderRadius: 16, padding: 24, textAlign: 'center', color: '#a8453f', fontSize: 14, fontWeight: 600 }}>
                  {usersError}
                </div>
              ) : !users.length ? (
                <div style={{ background: '#faf7f2', border: '1px dashed #e7dcc7', borderRadius: 20, padding: 48, textAlign: 'center' }}>
                  <div style={{ fontSize: 15, color: '#5d6b63', marginBottom: 8 }}>{usersLoading ? 'Loading candidates…' : 'No registered candidates yet.'}</div>
                  <div style={{ fontSize: 13, color: '#6b7872' }}>Once candidates register and start the advisor, they'll appear here.</div>
                </div>
              ) : (
                <div style={{ ...cardShell, overflow: 'hidden' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 110px 1fr 40px', gap: 0, padding: '10px 20px', borderBottom: '1px solid #f1eadd', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63' }}>
                    ...
                    return (
                      <button key={u.id} onClick={() => openCandidate(u.id)} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 110px 1fr 40px', gap: 0, padding: '18px 20px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', alignItems: 'center', borderBottom: '1px solid #f6f1e8' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <span style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(140deg,#1d3b32,#1d3b32)', color: '#faf7f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, flexShrink: 0 }}>{uInitials}</span>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: '#1a2420' }}>{u.name}</div>
                            <div style={{ fontSize: 12, color: '#5d6b63' }}>{[u.residency, u.email].filter(Boolean).join(' · ')}</div>
                          </div>
                        </div>
                        <div>
                          {u.scores ? (
                            <span style={{ fontSize: 22, fontWeight: 800, color: scoreColor(u.scores.overall) }}>{u.scores.overall}</span>
                          ) : <span style={{ fontSize: 13, color: '#6b7872' }}>—</span>}
                        </div>
                        <div>
                          <span style={{ background: '#f1eadd', color: '#1d3b32', fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 8 }}>{stepsFor(u.category)[u.stepIdx] || 'Profile'}</span>
                        </div>
                        <div style={{ fontSize: 13, color: '#2c3833', paddingRight: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {u.topInsight || (u.degree ? `${u.degree} candidate` : (u.sessionActive ? 'Session in progress' : 'Not started'))}
                        </div>
                        <div style={{ color: '#1d3b32', fontSize: 18, fontWeight: 700 }}>→</div>
                      </button>
                    );
                  })}
                </div>
          ...
          {adminView === 'candidates' && candidateOpen && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24 }}>
              <div>
                <button onClick={closeCandidate} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: '#1d3b32', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', padding: '0 0 20px', marginLeft: -4 }}>
                ...
                {selectedLoading && (
                  <div style={{ background: '#faf7f2', border: '1px dashed #e7dcc7', borderRadius: 20, padding: 32, textAlign: 'center', color: '#5d6b63', fontSize: 14, marginBottom: 20 }}>
                ...
                <div style={{ ...cardShell, padding: 24, marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <span style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(140deg,#c0844a,#a8453f)', color: '#faf7f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 18, flexShrink: 0 }}>{initials}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: '#1a2420' }}>{candidateName}</div>
                      <div style={{ fontSize: 13, color: '#5d6b63', marginTop: 2 }}>{candidateSub}</div>
                      {profile && (
                        <div style={{ fontSize: 12, color: '#5d6b63', marginTop: 4 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      {scores && (
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: 32, fontWeight: 800, color: '#1a2420', lineHeight: 1 }}>{scores.overall}</div>
                          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginTop: 2 }}>OVERALL</div>
                        </div>
                      )}
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 4 }}>PIPELINE STEP</div>
                        <span style={{ background: '#f1eadd', color: '#1d3b32', fontSize: 12, fontWeight: 700, padding: '5px 12px', borderRadius: 9 }}>{stepLabel}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {strengths && strengths.length > 0 && (
                  <div style={{ ...cardShell, padding: 22, marginBottom: 20 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 14 }}>KEY INSIGHTS</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {strengths.map((s, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#c0844a', marginTop: 6, flexShrink: 0 }} />
                          <span style={{ fontSize: 13.5, color: '#2c3833', lineHeight: 1.55 }}>{s}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {weaknesses && weaknesses.length > 0 && (
                  <div style={{ ...cardShell, border: '1px solid #f3e3df', padding: 22, marginBottom: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                      <span style={{ color: '#a8453f', fontSize: 15 }}>⚠</span>
                      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#a8453f' }}>RISK PROFILE</div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {weaknesses.map((w, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#a8453f', marginTop: 6, flexShrink: 0 }} />
                          <span style={{ fontSize: 13.5, color: '#2c3833', lineHeight: 1.55 }}>{w}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {programs && programs.length > 0 && (
                  <div style={{ ...cardShell, padding: 22 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 14 }}>SCHOOL PORTFOLIO — {programs.length} schools</div>
                    ...
                    {chosenPrograms.length > 0 && (
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#c0844a', marginBottom: 8 }}>★ CANDIDATE'S CHOSEN SCHOOLS</div>
                          ...
                          {chosenPrograms.map(p => (
                            <div key={p.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: tierBg(p.tier), border: `1.5px solid ${tierBorder(p.tier)}`, borderRadius: 12, padding: '9px 12px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ color: '#c0844a', fontSize: 13 }}>★</span>
                                <span style={{ fontSize: 13.5, fontWeight: 700, color: '#1a2420' }}>{p.name}</span>
                              </div>
                        ...
                        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 8 }}>FULL PORTFOLIO</div>
                      </div>
                    )}
                        ...
                        return (
                          <div key={p.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #f6f1e8' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <span style={{ fontSize: 13.5, fontWeight: isChosen ? 700 : 600, color: '#1a2420' }}>{p.name}</span>
                              {isChosen && <span style={{ fontSize: 10, background: '#e9c79a', color: '#c0844a', fontWeight: 700, padding: '2px 6px', borderRadius: 5, letterSpacing: '.3px' }}>CHOSEN</span>}
                            </div>
                            <span style={{ fontSize: 13, fontWeight: 700, color: '#c0844a' }}>{p.fit != null ? `${p.fit}%` : '—'}</span>
                          </div>
                        );
                      })}
                      {programs.length > Math.max(6, chosenPrograms.length) && (
                        <div style={{ fontSize: 12, color: '#5d6b63', marginTop: 4 }}>+{programs.length - Math.max(6, chosenPrograms.length)} more schools in full portfolio</div>
                      )}
                    </div>
                  </div>
                )}
              ...
              <div style={{ ...cardShell, padding: 22, marginTop: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: summaryVisible && summary ? 14 : 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63' }}>SESSION SUMMARY</div>
                ...
                {summaryVisible && (
                  <div style={{ marginTop: 14 }}>
                    {summarizing && !summary && (
                      <div style={{ fontSize: 13, color: '#5d6b63', fontStyle: 'italic' }}>Analyzing conversation…</div>
                    )}
                    {summary && (
                      <div style={{ fontSize: 13.5, lineHeight: 1.7, color: '#2c3833', whiteSpace: 'pre-wrap', background: '#f6f1e8', borderRadius: 14, padding: '14px 16px', border: '1px solid #f1eadd' }}>{renderFormattedText(summary)}</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div style={{ background: '#f6f1e8', border: '1px solid #f1eadd', borderRadius: 20, padding: 24, alignSelf: 'start', position: 'sticky', top: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '1px', color: '#5d6b63', marginBottom: 16 }}>CONSULTANT CONTROLS</div>
                <div style={{ marginBottom: 22 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#1a2420' }}>Score Override</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <button onClick={() => handleOverride(-1)} style={{ width: 30, height: 30, borderRadius: 9, border: '1px solid #f1eadd', background: '#faf7f2', color: '#1a2420', fontSize: 18, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', lineHeight: 1 }}>–</button>
                      <span style={{ fontSize: 18, fontWeight: 700, color: '#1a2420', minWidth: 32, textAlign: 'center' }}>{override}</span>
                      <button onClick={() => handleOverride(1)} style={{ width: 30, height: 30, borderRadius: 9, border: '1px solid #f1eadd', background: '#faf7f2', color: '#1a2420', fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', lineHeight: 1 }}>+</button>
                    </div>
                  </div>
                ...
                {/* Candidate documents */}
                <div style={{ marginBottom: 22 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 10 }}>CANDIDATE DOCUMENTS</div>
                  {documents.length === 0 ? (
                    <div style={{ background: '#faf7f2', border: '1px dashed #e7dcc7', borderRadius: 14, padding: '18px 14px', textAlign: 'center', fontSize: 12.5, color: '#5d6b63' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {documents.map(doc => (
                        <div key={doc.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#faf7f2', border: '1px solid #f1eadd', borderRadius: 12, padding: '10px 12px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ width: 28, height: 28, borderRadius: 8, background: '#f1eadd', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1d3b32', flexShrink: 0 }}>
                            ...
                            <div>
                              <div style={{ fontSize: 13, fontWeight: 700, color: '#1a2420' }}>{doc.label}</div>
                              <div style={{ fontSize: 11, color: '#5d6b63' }}>{doc.text.trim().split(/\s+/).length} words{doc.file ? ` · original: ${doc.file.name}` : ''}</div>
                            </div>
                          </div>
                ...
                {narrative && (
                  <div style={{ background: '#f1eadd', border: '1px solid #f1eadd', borderRadius: 14, padding: 14, marginBottom: 18 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 5 }}>NARRATIVE STRATEGY</div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: '#1a2420', textTransform: 'capitalize' }}>{narrative}</div>
                    <div style={{ fontSize: 13, color: '#5d6b63', marginTop: 3 }}>
                      {narrative === 'upgrade' ? 'Deepening existing trajectory' : 'Pivoting to new direction'}
                    </div>
                  </div>
                )}

                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 10 }}>SEND NOTE TO CANDIDATE</div>
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, background: '#faf7f2', border: '1px solid #f1eadd', borderRadius: 14, padding: '6px 6px 6px 14px', marginBottom: 0 }}>
                  <textarea value={msgInput} onChange={e => setMsgInput(e.target.value)} placeholder="Send a note to candidate's session..." rows="3"
                    style={{ flex: 1, border: 'none', outline: 'none', background: 'none', resize: 'none', fontSize: 13, fontFamily: 'inherit', color: '#1a2420', padding: '8px 0' }} />
          ...
          {adminView === 'users' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <div style={{ display: 'flex', gap: 18 }}>
                  <div style={{ ...cardShell, padding: '10px 18px' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63' }}>TOTAL USERS</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: '#1a2420' }}>{users.length}</div>
                  </div>
                  <div style={{ ...cardShell, padding: '10px 18px' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63' }}>ACTIVE</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: '#2f6b4f' }}>{users.filter(u => !u.suspended).length}</div>
                  </div>
                  <div style={{ ...cardShell, padding: '10px 18px' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63' }}>SUSPENDED</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: '#a8453f' }}>{users.filter(u => u.suspended).length}</div>
                  </div>
                </div>
                <button onClick={fetchUsers} disabled={usersLoading} style={{ ...btnGhost, padding: '7px 14px', fontSize: 12.5, cursor: usersLoading ? 'not-allowed' : 'pointer' }}>
                  {usersLoading ? 'Refreshing…' : 'Refresh'}
              ...
              {usersError ? (
                ...
                <div style={{ background: '#faf7f2', border: '1px dashed #e7dcc7', borderRadius: 20, padding: 48, textAlign: 'center' }}>
                  <div style={{ fontSize: 15, color: '#5d6b63' }}>{usersLoading ? 'Loading users…' : 'No registered users yet.'}</div>
                </div>
              ) : (
                <div style={{ ...cardShell, overflow: 'hidden' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr .8fr 1.6fr', gap: 0, padding: '10px 20px', borderBottom: '1px solid #f1eadd', fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63' }}>
                    ...
                    return (
                      <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr .8fr 1.6fr', gap: 0, padding: '16px 20px', alignItems: 'center', borderBottom: '1px solid #f6f1e8' }}>
                        <button onClick={() => setUserDetailId(u.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', padding: 0 }}>
                          <span style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(140deg,#1d3b32,#1d3b32)', color: '#faf7f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, flexShrink: 0 }}>{uInitials}</span>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: '#1a2420' }}>{u.name}</div>
                            <div style={{ fontSize: 12, color: '#5d6b63' }}>{u.email}</div>
                          </div>
                        </button>
                        <div style={{ fontSize: 13, color: '#2c3833' }}>{formatDateTime(u.lastLoginAt)}</div>
                        <div style={{ fontSize: 13, color: '#2c3833' }}>{formatDuration(u.sessionDurationMs)}</div>
                        <div>
                          <span style={{
                            fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 8, letterSpacing: '.3px',
                            background: u.suspended ? '#f3e3df' : '#cdd8d1',
                            color: u.suspended ? '#a8453f' : '#2f6b4f',
                            border: `1px solid ${u.suspended ? '#f3e3df' : '#bcccc4'}`,
                ...
                <div onClick={() => setUserDetailId(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(15,26,48,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 20 }}>
                  <div onClick={e => e.stopPropagation()} style={{ background: '#faf7f2', borderRadius: 22, padding: 28, width: 460, maxWidth: '100%', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(40,30,90,.28)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
                      <span style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(140deg,#1d3b32,#1d3b32)', color: '#faf7f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 17, flexShrink: 0 }}>
                        {(userDetail.name || '?').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()}
                      </span>
                      <div>
                        <div style={{ fontSize: 19, fontWeight: 800, color: '#1a2420' }}>{userDetail.name}</div>
                        <div style={{ fontSize: 13, color: '#5d6b63' }}>{userDetail.email}</div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                      {[
                        ...
                        <div key={label} style={{ background: '#f1eadd', borderRadius: 12, padding: '10px 12px' }}>
                          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.4px', color: '#5d6b63', marginBottom: 3 }}>{label.toUpperCase()}</div>
                          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#1a2420' }}>{val}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    {userDetail.loginHistory && userDetail.loginHistory.length > 0 && (
                      <div style={{ marginBottom: 22 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#5d6b63', marginBottom: 8 }}>RECENT LOGIN ACTIVITY</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 160, overflowY: 'auto' }}>
                          {[...userDetail.loginHistory].reverse().map((h, i) => (
                            <div key={i} style={{ fontSize: 12.5, color: '#2c3833', display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #f6f1e8', padding: '4px 0' }}>
                              <span>Login</span><span style={{ color: '#5d6b63' }}>{formatDateTime(h.at)}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <button onClick={() => setUserDetailId(null)} style={{ marginTop: 14, width: '100%', background: 'none', border: 'none', color: '#5d6b63', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', padding: 6 }}>
                      Close
                    </button>
                  </div>
                </div>
          ...
          {adminView === 'session' && !selectedUserId && (
            <div style={{ background: '#faf7f2', border: '1px dashed #e7dcc7', borderRadius: 20, padding: 48, textAlign: 'center' }}>
              <div style={{ fontSize: 15, color: '#5d6b63', marginBottom: 8 }}>No candidate selected.</div>
              <div style={{ fontSize: 13, color: '#6b7872' }}>Open a candidate from the Candidates list to view their live session.</div>
            </div>
          )}
          {adminView === 'session' && selectedUserId && (
            <div style={{ maxWidth: 800 }}>
              <div style={{ fontSize: 13, color: '#5d6b63', fontWeight: 600, marginBottom: 4 }}>Viewing: {candidateName}</div>
              {/* Summarize button */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div style={{ fontSize: 13, color: '#5d6b63', fontWeight: 600 }}>{chat.length} messages in session</div>
                <button onClick={generateSummary} disabled={summarizing || !sessionActive}
              ...
              {/* Summary card */}
              {summary && (
                <div style={{ background: '#e9c79a', border: '1px solid #e9c79a', borderRadius: 16, padding: '20px 24px', marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.5px', color: '#c0844a', marginBottom: 10 }}>SESSION SUMMARY</div>
                  <div style={{ fontSize: 14, lineHeight: 1.7, color: '#2c3833', whiteSpace: 'pre-wrap' }}>{renderFormattedText(summary)}</div>
                </div>
              )}

              {/* Full chat log */}
              {!sessionActive ? (
                <div style={{ background: '#faf7f2', borderRadius: 16, padding: 32, textAlign: 'center', color: '#5d6b63', border: '1px solid #f1eadd' }}>No session data. Have a candidate log in and start the advisor.</div>
              ) : (
                <div
                  style={{
                ...
                >
                  {chat.map((m, i) => (
                    <div key={i} style={{
                      ...
                      background: m.role === 'ai' ? '#f1eadd' : 'linear-gradient(135deg,#1d3b32,#1d3b32)',
                      flexShrink: 0,
                    }}>
                      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.5px', color: m.role === 'ai' ? '#5d6b63' : 'rgba(255,255,255,.75)', marginBottom: 4 }}>
                        {m.role === 'ai' ? 'AI ADVISOR' : candidateName.toUpperCase()}
                      </div>
                      <div style={{ fontSize: 13, lineHeight: 1.55, color: m.role === 'ai' ? '#2c3833' : '#faf7f2', whiteSpace: 'pre-wrap' }}>
          ...
          {adminView === 'settings' && (
            <div style={{ maxWidth: 700, display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div style={{ ...cardShell, padding: 28 }}>
                <h3 style={{ fontSize: 20, fontWeight: 800, color: '#1a2420', margin: '0 0 18px' }}>Portal Settings</h3>
                <div style={{ fontSize: 14, color: '#5d6b63', lineHeight: 1.6, marginBottom: 18 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13, color: '#5d6b63' }}>
              ...
              <div style={{ ...cardShell, padding: 28 }}>
                <h3 style={{ fontSize: 20, fontWeight: 800, color: '#1a2420', margin: '0 0 8px' }}>AI Process Configuration</h3>
                <div style={{ fontSize: 14, color: '#5d6b63', lineHeight: 1.6, marginBottom: 22 }}>
                ...
                {!aiConfigLoaded && (
                  <div style={{ fontSize: 13, color: '#5d6b63' }}>Loading configuration…</div>
                )}
                    ...
                    return (
                      <div key={s.key}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: '#1a2420' }}>{s.label}</div>
                          {isCustom && (
                            <span style={{ fontSize: 11, fontWeight: 700, color: '#c0844a', background: '#e9c79a', border: '1px solid #e9c79a', borderRadius: 7, padding: '2px 8px' }}>CUSTOM</span>
                          )}
                        </div>
                        <div style={{ fontSize: 12.5, color: '#5d6b63', marginBottom: 8, lineHeight: 1.5 }}>{s.description}</div>
                        <textarea
                          ...
                          style={{
                            ...
                            color: '#2c3833', background: '#f6f1e8', border: '1px solid #f1eadd', borderRadius: 14,
