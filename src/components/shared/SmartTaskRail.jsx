...
function TaskRow({ task, tone, onToggle, onOpen, now }) {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${overdue ? '#eccfd9' : '#eef0f7'}`, borderLeft: `3px solid ${tone}`, borderRadius: 11, padding: '9px 11px', minHeight: 58, boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button onClick={() => onOpen(task)} title={task.title || compactLabel(task)} style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: isDone ? '#9098b5' : '#2a3447', textDecoration: isDone ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{compactLabel(task)}</div>
          <div style={{ fontSize: 11, color: overdue ? '#e0556b' : '#9098b5', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
function TaskDetailsModal({ task, onClose, now }) {
  ...
  const overdue = !isDone && task.deadline && new Date(task.deadline).getTime() < now;
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(15,26,48,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 80, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fffdf7', borderRadius: 20, padding: 24, width: 460, maxWidth: '100%', maxHeight: '82vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(40,30,90,.28)' }}>
        <div style={{ display: 'inline-block', fontSize: 10.5, fontWeight: 800, letterSpacing: '.5px', color: '#fff', background: overdue ? '#e0556b' : '#5b46e0', borderRadius: 7, padding: '3px 9px', marginBottom: 12, textTransform: 'uppercase' }}>
          {overdue ? 'Overdue task' : 'Task'}
        </div>
        <div style={{ fontSize: 18, fontWeight: 800, color: '#141b34', marginBottom: 8, lineHeight: 1.35, overflowWrap: 'anywhere' }}>{task.title || compactLabel(task)}</div>
        <div style={{ fontSize: 13, color: '#6b7392', marginBottom: 4 }}>{task.deadline ? `Due · ${fmt(task.deadline)}` : 'No date'}</div>
        {task.priority && <div style={{ fontSize: 13, color: '#6b7392', marginTop: 4, textTransform: 'capitalize' }}>Priority · {task.priority}</div>}
        {task.status && <div style={{ fontSize: 13, color: '#6b7392', marginTop: 4, textTransform: 'capitalize' }}>Status · {task.status}</div>}
        {task.area && <div style={{ fontSize: 13, color: '#6b7392', marginTop: 4 }}>Area · {task.area}</div>}
        {detailText(task) && <div style={{ fontSize: 13.5, color: '#33405e', lineHeight: 1.55, marginTop: 14, background: '#faf7f2', border: '1px solid #efe7d4', borderRadius: 12, padding: 13, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{detailText(task)}</div>}
        <button onClick={onClose} style={{ marginTop: 18, width: '100%', background: '#141b34', color: '#fff', border: 'none', borderRadius: 11, padding: '11px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Close</button>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <div style={{ maxHeight: 360, overflowY: 'auto', overscrollBehavior: 'contain', paddingRight: 4, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <Group label="Due now" tasks={dueNow} tone="#e0556b" onToggle={onToggle} onOpen={setSelectedTask} now={now} empty="Nothing due right now." />
        <Group label="This week" tasks={thisWeek} tone="#c08a1a" onToggle={onToggle} onOpen={setSelectedTask} now={now} empty="Clear for the week." />
        <Group label="Important later" tasks={importantLater} tone="#5b46e0" onToggle={onToggle} onOpen={setSelectedTask} now={now} empty="No later tasks." />
        {completed.length > 0 && (
          <div>
            ...
            {showDone && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginTop: 8 }}>
                {completed.map(t => <TaskRow key={t.id} task={t} tone="#2f9e78" onToggle={onToggle} onOpen={setSelectedTask} now={now} />)}
              </div>
            )}
          </div>
        )}
