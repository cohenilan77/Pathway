    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <div style={{ flex: 1, background: '#eef1fc', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: 48 }}>
        <div style={{ width: '100%', maxWidth: 440 }}>
          ...
          {/* Email (candidates only — consultants use an access code instead) */}
          {role === 'candidate' && (
            <>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#1c2433', marginBottom: 8 }}>Email</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1px solid #d7ddec', borderRadius: 10, padding: '0 14px', marginBottom: 20 }}>
                ...
                <input
                  ...
                  style={{ flex: 1, border: 'none', outline: 'none', background: 'none', padding: '14px 0', fontSize: 15, color: '#1c2433', fontFamily: 'inherit' }}
                />
              </div>
            </>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <label style={{ fontSize: 13, fontWeight: 700, color: '#1c2433' }}>{role === 'candidate' ? 'Password' : 'Access Code'}</label>
            {role === 'candidate' && (
          ...
          {/* Enter button */}
          <button onClick={submit} disabled={authBusy} style={{ width: '100%', background: '#0f1a30', color: '#fff', border: 'none', borderRadius: 10, padding: 16, fontSize: 15, fontWeight: 700, cursor: authBusy ? 'not-allowed' : 'pointer', opacity: authBusy ? 0.7 : 1, fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          ...
          {role === 'candidate' && (
            <div style={{ textAlign: 'center', margin: '22px 0', fontSize: 14, color: '#1c2433' }}>
              New to the circle?{' '}
              <button onClick={() => go('register')} style={{ background: 'none', border: 'none', color: '#b8902f', fontWeight: 700, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit' }}>
                Request Access
              </button>
            </div>
          )}
