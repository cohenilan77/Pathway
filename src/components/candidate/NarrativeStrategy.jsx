...
export default function NarrativeStrategy({ narrative, setNarrative, setCandTab, send, noop }) {
  const narrCard = (kind) => {
    ...
    const accent = kind === 'pivot' ? '#faa984' : '#9285e4';
    const gradient = kind === 'pivot' ? 'linear-gradient(135deg,#fbc094,#fba2bb)' : 'linear-gradient(135deg,#94b3fb,#b899fb)';
    return {
      card: {
      ...
      iconBg: kind === 'pivot' ? '#fff1e8' : '#f1eadd',
      iconColor: kind === 'pivot' ? '#e18854' : '#9285e4',
      gradient,
      accent,
      on,
    };
    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 34px 64px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, background: '#faf7f2', border: '1px solid #f1eadd', borderRadius: 18, padding: '20px 22px', marginTop: 24, boxShadow: '0 18px 40px rgba(60,72,130,.06)' }}>
          <span style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(140deg,#94b3fb,#b899fb)', color: '#faf7f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, flexShrink: 0, boxShadow: '0 8px 16px rgba(105,91,255,.32)' }}>LS</span>
          <div>
