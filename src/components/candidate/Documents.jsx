...
export default function Documents({ docTab, setDocTab, cvText, setCvText, essayText, setEssayText, essaySchool, setEssaySchool, essayQuestion, setEssayQuestion, essays, interviews, selectEssaySchool, chosenSchools, insights, rewriteEssay, analyzeEssay, busy, setShowCvModal, setCandTab, send, showToast, narrative, isMobile }) {
  ...
  const mainContent = (
    <>
      {docTab === 'editor' && (
        <div style={{ background: '#fff', maxWidth: isMobile ? '100%' : 580, width: '100%', borderRadius: 8, boxShadow: isMobile ? 'none' : '0 10px 40px rgba(15,26,48,.08)', padding: isMobile ? '24px 20px' : '48px 52px', minHeight: isMobile ? 'auto' : 600, boxSizing: 'border-box' }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: isMobile ? 23 : 30, lineHeight: 1.2, fontWeight: 700, color: '#16233f', margin: '0 0 24px' }}>
      ...
      {docTab === 'documents' && (
        <div style={{ width: '100%', maxWidth: isMobile ? '100%' : 680 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: isMobile ? 22 : 28, fontWeight: 700, color: '#16233f', margin: 0 }}>My CV</h2>
            {cvText ? (
          ...
          {cvText ? (
            editingCv ? (
              ...
              <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #eaedf4', padding: isMobile ? '20px 18px' : '28px 32px', whiteSpace: 'pre-wrap', fontSize: 14, lineHeight: 1.8, color: '#33405a', fontFamily: 'inherit', minHeight: 400, boxSizing: 'border-box' }}>
                {cvText}
              </div>
            )
          ) : (
            <div style={{ background: '#fff', borderRadius: 10, border: '2px dashed #d7ddec', padding: isMobile ? 28 : 48, textAlign: 'center' }}>
      ...
      {docTab === 'interview' && (
        <div style={{ width: '100%', maxWidth: isMobile ? '100%' : 680 }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: isMobile ? 22 : 28, fontWeight: 700, color: '#16233f', margin: '0 0 8px' }}>Mock Interview</h2>
          ...
          {interviewSchools.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {interviewSchools.map(school => {
                ...
                return (
                  <div key={school} style={{ background: '#fff', borderRadius: 12, border: '1px solid #eaedf4', padding: isMobile ? '16px 18px' : '20px 22px' }}>
      ...
      {docTab === 'insights' && (
        <div style={{ width: '100%', maxWidth: isMobile ? '100%' : 580 }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: isMobile ? 22 : 28, fontWeight: 700, color: '#16233f', margin: '0 0 24px' }}>AI Insights</h2>
  ...
  if (isMobile) {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff', overflowY: 'auto' }}>
        {/* Sub-nav as horizontal tabs */}
        <div style={{ display: 'flex', gap: 6, padding: '14px 14px', borderBottom: '1px solid #eef1f6', overflowX: 'auto', flexShrink: 0 }}>
          {subNavItems.map(item => (
            <button key={item.key} onClick={() => setDocTab(item.key)} style={{ ...docNavStyle(docTab === item.key), width: 'auto', flexShrink: 0, padding: '9px 14px' }}>
        ...
        {/* Main content */}
        <div style={{ background: '#f6f7fb', padding: '20px 16px', flex: 1 }}>
          {mainContent}
        </div>

        {/* Strategist tools, stacked below content on mobile */}
        <div style={{ borderTop: '1px solid #eef1f6', background: '#fbfcfe' }}>
          <div style={{ background: '#eef1fc', padding: 16, borderBottom: '1px solid #e1e6f5' }}>
