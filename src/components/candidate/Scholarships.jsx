...
const statusColor = (status) => ({
  suggested: { bg: '#f2f6ff', color: '#3a63ff' },
  interested: { bg: '#f2f6ff', color: '#111a33' },
  applying: { bg: '#ffffff', color: '#b45309' },
  applied: { bg: '#e9f0ff', color: '#2f52e6' },
  awarded: { bg: '#e6faf3', color: '#0ca678' },
  declined: { bg: '#ffe9ef', color: '#e8476b' },
}[status] || { bg: '#f2f6ff', color: '#111a33' });
...
export default function Scholarships({ authToken, showToast, chosenSchools, programs, profile, scores }) {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, padding: '24px 28px 28px', background: '#eef4ff', overflowY: 'auto' }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: '#111a33', margin: '0 0 4px', letterSpacing: '-.4px' }}>Scholarships</h1>
        <div style={{ fontSize: 13, color: '#5a6a8f', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
          <button
            ...
            disabled={searching}
            style={{ border: 'none', borderRadius: 999, padding: '10px 18px', fontFamily: 'inherit', fontSize: 13, fontWeight: 700, color: '#fff', background: 'linear-gradient(135deg,#3a63ff,#6d8cff)', cursor: searching ? 'wait' : 'pointer', opacity: searching ? 0.65 : 1 }}
          >
          ...
          {searchSchoolNames.length > 0 && (
            <span style={{ fontSize: 12.5, color: '#5a6a8f' }}>Searches {searchSchoolNames.length} school{searchSchoolNames.length === 1 ? '' : 's'} on your list.</span>
          )}
        </div>

        {error && <div style={{ padding: '12px 16px', borderRadius: 10, background: '#ffe9ef', color: '#e8476b', fontSize: 13.5, marginBottom: 16 }}>{error}</div>}

        {scholarships === null && !error && (
          <div style={{ color: '#5a6a8f', fontSize: 13.5 }}>Loading…</div>
        )}

        {scholarships && scholarships.length === 0 && (
          <div style={{ padding: '28px 24px', borderRadius: 16, background: '#f2f6ff', border: '1px solid #dbe4f7', color: '#5a6a8f', fontSize: 13.5, textAlign: 'center' }}>
        ...
        {scholarships && scholarships.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {scholarships.map((s) => {
              ...
              return (
                <div key={s.id || s.name} style={{ padding: '16px 18px', borderRadius: 14, background: '#f2f6ff', border: '1px solid #dbe4f7' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <a href={s.url} target="_blank" rel="noreferrer" style={{ fontSize: 15, fontWeight: 800, color: '#111a33', textDecoration: 'none' }}>{s.name}</a>
                      ...
                      <div style={{ fontSize: 12.5, color: '#5a6a8f', marginTop: 4 }}>
                        {s.amountUSD ? `$${Number(s.amountUSD).toLocaleString()}` : ''}
                        {s.amountUSD && s.deadline ? ' · ' : ''}
                        {s.deadline ? `Deadline: ${s.deadline}` : ''}
                      </div>
                      {s.eligibility && <div style={{ fontSize: 12.5, color: '#5a6a8f', marginTop: 4 }}>{s.eligibility}</div>}
                    </div>
                    <select
