...
const OPTIONS_PATTERN = /→\s*(.+)$/;
  ...
  const scrollButtonStyle = {
    ...
    boxShadow: '0 8px 18px rgba(58,99,255,.36)',
  };

  return (
    <div dir={dir} className="pw-chat-page" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '30px 36px' }}>
      <div style={{ maxWidth: 800 }}>
        <div style={{ fontSize: 13, color: '#8b97b8', fontWeight: 600, marginBottom: 14, textAlign: dir === 'rtl' ? 'right' : 'left' }}>{chatT(language, 'viewingConsultant')}</div>
        <div style={{ position: 'relative', background: '#f2f6ff', border: '1px solid #dbe4f7', borderRadius: 20, boxShadow: '0 18px 40px rgba(30,45,90,.06)', display: 'flex', flexDirection: 'column', height: '65vh', minHeight: 360, maxHeight: 680, overflow: 'hidden' }}>
          {showScrollTop && (
            <>
          ...
          <div ref={chatScrollRef} className="pw-chat-messages" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '24px 28px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 640 }}>
              {loading && (
                <div style={{ fontSize: 13.5, color: '#97a3c0', textAlign: 'center', padding: '40px 0' }}>
              ...
              {messages.filter(m => m.senderRole !== 'system' && m.senderRole !== 'ai').map((m) => (
                m.senderRole === 'candidate' ? (
                  <div key={m.id} style={{ alignSelf: 'flex-end', background: 'linear-gradient(135deg,#3a63ff,#6d8cff)', color: '#f2f6ff', borderRadius: '18px 18px 6px 18px', padding: '14px 19px', fontSize: 14.5, lineHeight: 1.55, maxWidth: '82%', whiteSpace: 'pre-wrap', boxShadow: '0 10px 22px rgba(58,99,255,.28)' }}>
                  ...
                  (() => {
                    ...
                    return (
                      <div key={m.id} style={{ alignSelf: 'flex-start', background: '#eef4ff', border: '1px solid #dbe4f7', borderRadius: '6px 18px 18px 18px', padding: '16px 19px', fontSize: 14.5, lineHeight: 1.62, color: '#38456b', whiteSpace: 'pre-wrap', maxWidth: '90%' }}>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                            {parsed.options.map((opt) => (
                              <button
                                ...
                                onClick={() => send(opt)}
                                disabled={sending}
                                style={{ background: '#fff', border: '1.5px solid #97a3c0', borderRadius: 999, padding: '7px 14px', fontSize: 13.5, fontWeight: 600, color: '#38456b', cursor: sending ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}
                              >
                                {opt}
                              </button>
                            ))}
          ...
          <div className="pw-chat-composer" style={{ padding: '16px 24px 20px', flexShrink: 0, borderTop: '1px solid #dbe4f7' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#eef4ff', border: '1.5px solid #e3ebfa', borderRadius: 18, padding: '7px 7px 7px 8px' }}>
              <input
                dir={dir}
                value={input}
                ...
                style={{ flex: 1, border: 'none', outline: 'none', background: 'none', fontSize: 14.5, padding: '11px 12px', color: '#1a2540', fontFamily: 'inherit', fontWeight: 500, textAlign: dir === 'rtl' ? 'right' : 'left' }}
              />
              <button onClick={() => send()} disabled={sending || !input.trim()} aria-label={chatT(language, 'send')}
                style={{ background: 'linear-gradient(135deg,#3a63ff,#6d8cff)', border: 'none', borderRadius: 13, width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: sending || !input.trim() ? 'not-allowed' : 'pointer', color: '#f2f6ff', flexShrink: 0, boxShadow: '0 8px 18px rgba(58,99,255,.36)', opacity: sending || !input.trim() ? 0.55 : 1 }}>
