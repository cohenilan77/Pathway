...
export default function UndergradKpiPanel({ scores = {}, profile = {}, compact = false }) {
  ...
  return (
    <section style={{ background: '#ffffff', border: '1px solid #e3ebfa', borderRadius: 18, padding: compact ? 14 : 18, boxShadow: '0 10px 28px rgba(30,45,90,.05)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '.6px', color: '#3a63ff', textTransform: 'uppercase' }}>Undergraduate readiness</div>
        <div style={{ fontSize: 14, fontWeight: 800, color: '#111a33' }}>Overall: {overall == null ? 'Needs update' : `${overall}%`}</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${compact ? 2 : 4},minmax(0,1fr))`, gap: 8 }} className="pw-undergrad-kpi-grid">
        {items.map(([label, keys]) => {
          ...
          return (
            <div key={label} style={{ background: '#f2f6ff', borderRadius: 11, padding: '10px 11px', minWidth: 0 }}>
              <div style={{ fontSize: 11.5, color: '#5a6a8f', lineHeight: 1.25 }}>{label}</div>
              <div style={{ fontSize: 17, fontWeight: 800, color: value == null ? '#8b97b8' : '#111a33', marginTop: 4 }}>{value == null ? 'Needs update' : `${value}%`}</div>
            </div>
          );
        })}
      </div>
