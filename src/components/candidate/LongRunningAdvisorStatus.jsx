...
export default function LongRunningAdvisorStatus({ busy, message, showDots = true }) {
  ...
  return (
    <span
      role="status"
      aria-live="polite"
      style={{ display: 'inline-flex', alignItems: 'center', gap: 9, minWidth: 0, fontSize: 12.5, fontWeight: 600, color: '#7a8295', lineHeight: 1.4, whiteSpace: 'nowrap', flexShrink: 0, animation: 'pwFade .25s ease' }}
    >
      {showDots && (
        <span aria-hidden="true" style={{ display: 'inline-flex', gap: 4, alignItems: 'center', flexShrink: 0 }}>
          {[0, 1, 2].map(i => (
            <span key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: '#5b46e0', display: 'inline-block', animation: `pwPulse 1.2s ease-in-out ${i * 0.2}s infinite` }} />
          ))}
        </span>
      )}
      <span>{label}</span>
