...
const CREAM = '#f2f6ff';
const CREAM_2 = '#eef4ff';
const BORDER = '#dbe4f7';
const BORDER_2 = '#e3ebfa';
const INK = '#111a33';
const BODY = '#38456b';
const MUTED = '#5a6a8f';
const MUTED_2 = '#8b97b8';
const VIOLET = '#3a63ff';
const GOLD = '#3a63ff';
const PERI = 'linear-gradient(135deg,#3a63ff,#6d8cff)';
const HAIRLINE = 'linear-gradient(180deg,#3a63ff,#6d8cff)';

const TIER_COLORS = {
  red: { fg: '#e8476b', bg: '#ffe9ef', bar: '#e8476b' },
  yellow: { fg: '#e08600', bg: '#ffffff', bar: '#e08600' },
  green: { fg: '#0ca678', bg: '#e6faf3', bar: '#0ca678' },
  grey: { fg: MUTED_2, bg: BORDER, bar: '#97a3c0' },
};
          ...
          <React.Fragment key={label}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              <span style={{
                ...
                ...(active
                  ? { background: INK, color: '#fff', boxShadow: '0 4px 10px rgba(30,45,90,.28)' }
                  : done
                  ? { background: INK, color: '#fff' }
                  : future
                  ? { background: '#ffffff', color: GOLD, border: '1px dashed #fff4e2' }
            ...
            {i < STEPS.length - 1 && <span style={{ width: 30, height: 1, background: done ? '#fff4e2' : BORDER_2, flexShrink: 0, margin: '0 10px' }} />}
          </React.Fragment>
        );
      })}
    </div>
    ...
    <div style={{ textAlign: 'center' }}>
      <div style={{ position: 'relative', width: size, height: size, margin: '0 auto' }}>
        <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
          <circle cx={center} cy={center} r={r} stroke="#eef4ff" strokeWidth={stroke} fill="none" />
          {has && (
            <circle
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bricolage Grotesque',serif", fontSize: 36, fontWeight: 600, color: has ? INK : MUTED_2 }}>
          {has ? Math.round(pct) : '—'}
        </div>
      </div>
...
function ToolStatusLine({ busy, message }) {
  ...
  return (
    <div role="status" aria-live="polite" style={{ display: 'flex', alignItems: 'center', gap: 8, paddingLeft: 18, whiteSpace: 'nowrap' }}>
      ...
      <span style={{ fontSize: 13, background: `linear-gradient(90deg,${MUTED} 35%,#6d8cff 50%,${MUTED} 65%)`, backgroundSize: '200% 100%', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent', animation: 'adv2Shimmer 1.8s linear infinite' }}>{label}</span>
    </div>
  );
}
...
function ProvenanceChip({ children }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, color: MUTED, background: 'rgba(255,255,255,.9)', border: `1px solid ${BORDER_2}`, borderRadius: 999, padding: '4px 10px' }}>
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M6 1.2 10 2.8v3c0 2.6-1.7 4.3-4 5-2.3-.7-4-2.4-4-5v-3L6 1.2Z" stroke="#0ca678" strokeWidth="1.2" strokeLinejoin="round" /><path d="M4.2 6.1 5.5 7.4l2.3-2.8" stroke="#0ca678" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
      {children}
    </span>
  );
}
...
function AdvisorAvatar({ size = 46 }) {
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: PERI, padding: 2, flexShrink: 0, boxShadow: '0 6px 16px rgba(58,99,255,.24)' }}>
...
function ProgramCard({ program, pickIndex, showRank, pending, onChoose, chosen }) {
  ...
  return (
    <div style={{ display: 'flex', background: '#fff', border: `1px solid ${isChosen ? '#6d8cff' : BORDER}`, borderLeft: `4px solid ${c.bar}`, borderRadius: 18, boxShadow: '0 1px 2px rgba(30,45,90,.04),0 8px 24px rgba(30,45,90,.05)', overflow: 'hidden', opacity: locked ? 0.72 : 1 }}>
      <div style={{ flex: 'none', display: 'flex', alignItems: 'flex-start', padding: '15px 0 15px 14px' }}>
        <button
          ...
          style={{
            ...
            ...(locked
              ? { border: '1.5px solid #dbe4f7', background: BORDER, color: '#8b97b8' }
              : isChosen
              ? { border: 'none', background: PERI, color: '#fff', boxShadow: '0 2px 7px rgba(58,99,255,.45)', animation: 'adv2Pop .4s ease' }
              : { border: '1.5px solid #c6d2ea', background: '#fff', color: '#6d8cff' }),
          }}
        >
          {isChosen
              ...
              <div style={{ marginTop: 14 }}>
                ...
                {[...risks, ...(risks.length ? [] : drivers), ...gaps].map((r, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, fontSize: 13, lineHeight: 1.55, marginBottom: 5 }}>
                    <span style={{ flex: 'none', width: 5, height: 5, borderRadius: '50%', background: risks.length ? '#e8476b' : '#0ca678', marginTop: 7 }} />
                    <span>{r}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
                <button
                  ...
                  style={{
                    ...
                    ...(isChosen
                      ? { border: 'none', background: PERI, color: '#fff', boxShadow: '0 3px 10px rgba(58,99,255,.4)', animation: 'adv2Pop .45s ease' }
                      : { border: '1px solid #6d8cff', background: '#fff', color: VIOLET }),
                  }}
                >
...
function AnalysisRail({ scores, gauges, fitIndex, hasScores }) {
  return (
    <aside className="pw-advisor-rail" style={{ background: 'rgba(255,255,255,.55)', borderLeft: `1px solid ${BORDER}`, padding: '26px 24px', overflowY: 'auto', minHeight: 0 }}>
      <div style={{ fontFamily: "'Bricolage Grotesque',serif", fontSize: 25, fontWeight: 600, color: INK, letterSpacing: '-.01em', lineHeight: 1.15 }}>Real-time<br />Analysis</div>
      ...
      <div style={{ marginTop: 26, borderTop: `1px solid ${BORDER}`, paddingTop: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
        ...
        <span style={{ fontFamily: "'Bricolage Grotesque',serif", fontSize: 22, fontWeight: 600, color: fitIndex != null ? INK : MUTED_2 }}>{fitIndex != null ? `${fitIndex}%` : '—'}</span>
      </div>
      {!hasScores && (
...
export default function AdvisorConversational({
  ...
  const gauges = [
    ...
    { label: 'Strategy', sublabel: 'Narrative & goals', color: '#3a63ff', value: pickScore(scores, ['narrative', 'goalClarity', 'potential']) },
  ];
    ...
    <div style={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', background: 'transparent', fontFamily: "'Albert Sans',system-ui,sans-serif", color: BODY, position: 'relative' }}>
      ...
      <div className="pw-advisor-status" style={{ flex: 'none', width: 'min(820px,calc(100% - 56px))', alignSelf: 'flex-start', marginLeft: 28, paddingTop: 6, boxSizing: 'border-box' }}>
        <button
          ...
          style={{ display: 'flex', width: '100%', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,.72)', border: `1px solid ${BORDER}`, borderRadius: 14, padding: '10px 14px', boxShadow: '0 1px 2px rgba(30,45,90,.03)', cursor: 'pointer', font: 'inherit', textAlign: 'left', color: BODY }}
        >
          <span style={{ fontSize: 12.5, fontWeight: 700, color: INK }}>Stage {journeyStageIndex + 1} of 5</span>
          <span style={{ width: 3, height: 3, borderRadius: '50%', background: '#97a3c0' }} />
          ...
          <span style={{ flex: 1 }} />
          <span style={{ width: 20, height: 20, borderRadius: '50%', background: `conic-gradient(${VIOLET} ${Math.max(0, Math.min(100, fitIndex || 0)) * 3.6}deg,#eef4ff 0)`, WebkitMask: 'radial-gradient(closest-side,transparent 62%,#000 66%)', mask: 'radial-gradient(closest-side,transparent 62%,#000 66%)' }} />
          ...
          <span aria-hidden="true" style={{ display: 'inline-flex', marginLeft: 2, transform: `rotate(${statusExpanded ? 180 : 0}deg)`, transition: 'transform .2s ease' }}>
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="#8b97b8" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </span>
        </button>
        {statusExpanded && (
          <div id="candidate-journey-stages" className="pw-adv2-stage-grid" style={{ marginTop: 8, animation: reduceMotion ? 'none' : 'adv2In .22s ease both' }}>
            {journeyStages.map((stageName, index) => {
              ...
              return (
                <div key={stageName} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', minHeight: 88, background: 'rgba(255,255,255,.72)', borderRadius: 14, padding: 12, border: current ? '1px solid #6d8cff' : `1px solid ${BORDER}`, boxShadow: current ? '0 4px 14px rgba(58,99,255,.18)' : 'none' }}>
                  <span style={{ width: 22, height: 22, borderRadius: '50%', background: done ? '#e6faf3' : current ? PERI : CREAM_2, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: done ? '#0ca678' : current ? '#fff' : MUTED_2, fontSize: 12, fontWeight: 700 }}>{done ? '✓' : index + 1}</span>
        ...
        <section className="pw-advisor-conversation" style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <div style={{ flex: 'none', display: 'flex', alignItems: 'center', gap: 14, padding: '20px 20px 6px' }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontFamily: "'Bricolage Grotesque',serif", fontSize: 26, fontWeight: 600, color: INK, letterSpacing: '-.01em', lineHeight: 1.15 }}>{headerTitle}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#0ca678', boxShadow: '0 0 8px rgba(12,166,120,.6)' }} />
                <span style={{ fontSize: 12.5, fontWeight: 700, color: '#0ca678' }}>Lead Admissions Strategist · Online</span>
              </div>
            </div>
          </div>
          ...
          <div ref={scrollRef} style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '18px 20px 12px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <React.Fragment key={i}>
                  ...
                  <div style={{ animation: reduceMotion ? 'none' : 'adv2In .3s ease both' }}>
                    {isAi ? (
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 14 }}>
                            {parsed.options.map((opt) => (
                              <button
                                ...
                                disabled={busy}
                                className="pw-adv2-btn"
                                style={{ background: '#fff', border: '1.5px solid #97a3c0', borderRadius: 12, padding: '9px 18px', fontSize: 14, fontWeight: 700, color: INK, cursor: busy ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}
                              >
                                {opt}
                              </button>
                            ))}
                            {showEndForNow && i === visibleChat.length - 1 && (
                              <button
                                ...
                                disabled={busy}
                                className="pw-adv2-btn"
                                style={{ background: '#f2f6ff', border: '1.5px solid #e3ebfa', borderRadius: 12, padding: '9px 18px', fontSize: 14, fontWeight: 700, color: MUTED_2, cursor: busy ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}
                              >
                                End for now
                              </button>
                            )}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <div style={{ maxWidth: '78%', background: PERI, color: '#fff', fontSize: 14.5, lineHeight: 1.55, padding: '11px 16px', borderRadius: '18px 18px 5px 18px', boxShadow: '0 4px 14px rgba(58,99,255,.3)', whiteSpace: 'pre-wrap' }}>
                  ...
                  {milestone && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '6px 0' }}>
                      <span style={{ flex: 1, height: 1, background: 'linear-gradient(90deg,transparent,#c6d2ea)' }} />
                  ...
                  {ghost && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, justifyContent: 'flex-end', opacity: .55, pointerEvents: 'none' }} aria-hidden="true">
                      {ghost.options.map(label => (
                        <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, borderRadius: 999, padding: '8px 14px', fontSize: 13, fontWeight: 600, border: `1px solid ${label === ghost.picked ? '#6d8cff' : BORDER_2}`, background: label === ghost.picked ? 'rgba(58,99,255,.14)' : 'none', color: label === ghost.picked ? VIOLET : MUTED_2 }}>
                  ...
                  {showNarrativeCtaHere && (
                    <div style={{ maxWidth: 640, marginLeft: 18, background: INK, borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, animation: reduceMotion ? 'none' : 'adv2In .3s ease both' }}>
                      ...
                      <button onClick={() => setShowNarrativeModal(true)} style={{ background: '#f2a63b', color: '#e08600', border: 'none', borderRadius: 9, padding: '9px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>Start →</button>
                    </div>
                  )}
                </React.Fragment>
              );
            ...
            {/* Live status widgets that genuinely reflect the CURRENT
                ...
                unlike these it isn't a live status, it's a one-time reveal. */}
                  {showProgramRecovery && (
              <div style={{ maxWidth: 640, marginLeft: 18, background: '#ffffff', border: `1px solid ${BORDER_2}`, borderRadius: 16, padding: '16px 18px', boxShadow: '0 6px 18px rgba(30,45,90,.06)' }}>
          ...
          <div style={{ flex: 'none', padding: '6px 20px 18px' }}>
            <div className="pw-composer-shell" style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: `1px solid ${BORDER_2}`, borderRadius: 20, padding: '8px 8px 8px 18px', boxShadow: '0 2px 6px rgba(30,45,90,.05),0 12px 32px rgba(30,45,90,.06)' }}>
              <input
                ref={inputRef}
                value={input}
              ...
              />
              {cvText ? (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: '#e6faf3', borderRadius: 999, padding: '9px 15px', fontSize: 13, fontWeight: 600, color: '#0ca678', whiteSpace: 'nowrap' }}>
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2.5 6.5 5 9l4.5-6" stroke="#0ca678" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  CV received
                </span>
              ) : (
                <button
              ...
              <button
                ...
                style={{ width: 38, height: 38, borderRadius: '50%', border: 'none', background: INK, cursor: busy || !input.trim() ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 10px rgba(30,45,90,.28)', opacity: busy || !input.trim() ? 0.55 : 1, flexShrink: 0 }}
              >
                ...
                {chips.map((c, i) => (
                  <button
                    ...
                    style={{
                      ...
                      border: i === 0 ? '1px solid #6d8cff' : `1px solid ${BORDER_2}`,
                      background: i === 0 ? '#fff' : 'rgba(255,255,255,.7)',
                      color: i === 0 ? VIOLET : BODY,
                      boxShadow: i === 0 ? '0 2px 8px rgba(58,99,255,.22)' : 'none',
                    }}
                  >
