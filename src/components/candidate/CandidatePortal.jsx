...
function navStyle(active) {
  return active
    ? { position: 'relative', display: 'flex', alignItems: 'center', gap: 13, padding: '12px 15px', borderRadius: 15, fontSize: 14, fontWeight: 700, cursor: 'pointer', width: '100%', textAlign: 'left', border: 'none', fontFamily: 'inherit', color: '#fff', background: 'linear-gradient(135deg,#4d83ff,#8a52ff)', boxShadow: '0 12px 24px rgba(105,91,255,.36), inset 0 1px 0 rgba(255,255,255,.32)' }
...
function navIconStyle(active) {
  return active
    ? { width: 32, height: 32, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: 'rgba(255,255,255,.18)', color: '#fff' }
    : { width: 32, height: 32, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: '#f3f5fc', color: '#8a93b3' };
}
function navDotStyle(active) {
  return active
    ? { marginLeft: 'auto', width: 7, height: 7, borderRadius: '50%', background: '#fff', boxShadow: '0 0 8px rgba(255,255,255,.8)' }
...
export default function CandidatePortal(props) {
  ...
  return (
    <div className="pw-shell" style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f3f5fc', fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif", color: '#141b34', WebkitFontSmoothing: 'antialiased' }}>
      ...
      <div className={`pw-sidebar${menuOpen ? ' pw-sidebar-open' : ''}`} style={{ width: 266, flexShrink: 0, height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', padding: '26px 18px', background: '#ffffff', borderRight: '1px solid #edf0f9' }}>
        ...
        <div style={{ position: 'relative', marginTop: 24 }}>
          <select
            ...
            style={{ width: '100%', appearance: 'none', border: '1.5px solid #ecf0fa', borderRadius: 14, padding: '12px 15px', fontSize: 13.5, fontWeight: 600, color: '#3c4564', background: '#f7f9fe', fontFamily: 'inherit', cursor: 'pointer', outline: 'none', boxSizing: 'border-box' }}
          >
        ...
        <div style={{ marginTop: 'auto' }}>
          <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 20, padding: '17px 17px 18px', background: 'linear-gradient(145deg,#5566ff,#9a52ff)', boxShadow: '0 16px 30px rgba(105,91,255,.34)' }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                <svg viewBox="0 0 24 24" width="15" height="15" style={{ fill: '#ffd76a', stroke: 'none' }}><path d="M12 2l2.6 6.3L21 9l-5 4.4L17.5 21 12 17.3 6.5 21 8 13.4 3 9l6.4-.7L12 2Z" /></svg>
                <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.4px', color: '#eae6ff' }}>{PLAN_LABELS[plan] || 'Pathway AI'}</span>
              </div>
              {plan !== 'aiStrategist' ? (
                <>
                  <div style={{ fontSize: 12.5, color: '#e3ddff', lineHeight: 1.45, marginBottom: 13, fontWeight: 500 }}>Unlock a dedicated strategist &amp; unlimited reviews.</div>
                  <button onClick={handleUpgrade} style={{ width: '100%', background: '#fff', color: '#5b46e0', border: 'none', borderRadius: 12, padding: 11, fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 6px 14px rgba(40,20,90,.18)' }}>Upgrade plan</button>
                </>
              ) : (
                <div style={{ fontSize: 12.5, color: '#e3ddff', lineHeight: 1.45, fontWeight: 500 }}>You have full access, including your dedicated strategist.</div>
              )}
            </div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginTop: 16, padding: '8px 8px', borderRadius: 14 }}>
            <div style={{ width: 38, height: 38, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(140deg,#ffb35c,#ff7a9c)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#fff', boxShadow: '0 6px 14px rgba(255,122,156,.35)' }}>{initials}</div>
            ...
            <button onClick={handleSignOut} title="Sign out" style={{ background: '#f1f3fb', border: 'none', borderRadius: 10, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#8a93b3', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '26px 36px', borderBottom: '1px solid #eef1f9', background: '#fff', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={handleHelp} title="Help" style={{ width: 42, height: 42, borderRadius: 13, border: '1.5px solid #ebeef8', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b7392' }}>
              <svg viewBox="0 0 24 24" width="18" height="18" style={{ fill: 'none', stroke: 'currentColor', strokeWidth: '1.9', strokeLinecap: 'round', strokeLinejoin: 'round' }}><circle cx="12" cy="12" r="9" /><path d="M9.5 9a2.5 2.5 0 0 1 4.9.5c0 1.7-2.4 2-2.4 3.5M12 17h.01" /></svg>
            </button>
            <button style={{ width: 42, height: 42, borderRadius: 13, border: '1.5px solid #ebeef8', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b7392', position: 'relative' }}>
              ...
              <span style={{ position: 'absolute', top: 9, right: 10, width: 8, height: 8, borderRadius: '50%', background: '#ff6b8b', border: '2px solid #fff' }} />
            </button>
            <button onClick={resetSession} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1.5px solid #ebeef8', borderRadius: 13, padding: '0 16px', height: 42, fontSize: 13, fontWeight: 700, color: '#5b46e0', cursor: 'pointer', fontFamily: 'inherit' }}>
